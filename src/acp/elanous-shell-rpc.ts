// ── A1 (Phase 2 Bundle 3) — ACP custom JSON-RPC: elanous shell as child ──
//
// HANDOFF Phase 2 §5 A1 / ROADMAP §5: "Codex 가 elanous shell 을 child
// terminal". 외부 ACP client (cursor / zed / codex) 가 elanous 의 shell 을
// borrow 해서 명령 실행 + 결과 읽기.
//
// Custom JSON-RPC method 4 개 정의:
//
//   elanous/shell.spawn   — 새 shell 생성 + handle id 반환
//   elanous/shell.write   — 실행 중 shell 에 stdin/keystroke 전달
//   elanous/shell.read    — 현재까지 누적 output snapshot
//   elanous/shell.close   — kill + unregister
//
...
export interface ShellSpawnRequest {
  ...
  /** Free-form description shown in elanous UI / status. */
...
export interface ElanousShellRpcDeps extends ShellRpcPolicyDeps {
...
export interface ElanousShellRpcHandlers {
...
export function createElanousShellRpcHandlers(deps: ElanousShellRpcDeps): ElanousShellRpcHandlers {
...
export const ELANOUS_SHELL_RPC_METHODS = {
  spawn: 'elanous/shell.spawn',
  write: 'elanous/shell.write',
  read: 'elanous/shell.read',
  close: 'elanous/shell.close',
} as const;
