...
// to render Monad's extended UI surface (TUI, Web, iPhone). It has
...
export interface MonadUiHandler {
  onShowModal?(payload: MonadUiShowModalPayload): void | Promise<void>;
  onShowToast?(payload: MonadUiShowToastPayload): void | Promise<void>;
  onUpdateStatusPill?(payload: MonadUiUpdateStatusPillPayload): void | Promise<void>;
  onUsage?(payload: MonadUiUsagePayload): void | Promise<void>;
}
  ...
  switch (parsed.method) {
    case 'showModal': {
      if (handler.onShowModal) {
        await handler.onShowModal(parsed.payload as unknown as MonadUiShowModalPayload);
      }
      return { kind: 'ui', method: 'showModal' };
    }
    case 'showToast': {
      if (handler.onShowToast) {
        await handler.onShowToast(parsed.payload as unknown as MonadUiShowToastPayload);
      }
      return { kind: 'ui', method: 'showToast' };
    }
    case 'updateStatusPill': {
      if (handler.onUpdateStatusPill) {
        await handler.onUpdateStatusPill(parsed.payload as unknown as MonadUiUpdateStatusPillPayload);
      }
      return { kind: 'ui', method: 'updateStatusPill' };
    }
    case 'usage': {
      if (handler.onUsage) {
        await handler.onUsage(parsed.payload as unknown as MonadUiUsagePayload);
      }
