...
/**
 * ⛔⭐⭐⭐ **정적 래칫 — `MONAD_STATE_DIR` 을 자식 env 에 세우는 자리엔 «출처»가 같이 있어야 한다.**
 *
...
describe('래칫 — 자식 env 에 뿌리를 세우면 «출처»도 세운다', () => {
  test('MONAD_STATE_DIR 을 쓰는 파일은 MONAD_STATE_DIR_SOURCE 도 쓴다', () => {
    ...
    for (const file of walk(join(REPO_ROOT, 'src'))) {
      ...
      if (!/MONAD_STATE_DIR:\s/.test(text)) continue;
      if (!text.includes('MONAD_STATE_DIR_SOURCE')) offenders.push(rel);
    }
  ...
  test('⛔ 래칫이 «아무것도 안 보고» 통과하지 않는다 — 대상 파일이 실제로 있다', () => {
    ...
    for (const file of walk(join(REPO_ROOT, 'src'))) {
      const rel = file.slice(REPO_ROOT.length + 1);
      if (EXEMPT.has(rel)) continue;
      if (/MONAD_STATE_DIR:\s/.test(readFileSync(file, 'utf8'))) seen++;
    }
    // 실측 2026-08-19: seams · headless-monad-driver · monad-tui-spawn · pty-drive-cli
