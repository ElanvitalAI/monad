...
// 로직을 만들지 않는다 — `monad config sync-test` 가 쓰는 그 함수 그대로다(재발명 0).
...
export interface ProvisionOpts {
  /** 스포너가 자식에게 명시로 넘기는 우주(`--config-dir` / `MONAD_STATE_DIR`).
...
function sealFromGit(root: string): boolean {
  try {
    ...
    const header = cur
      ? cur.replace(/\n*$/, '\n')
      : '# monad 파생 test 우주 — 이 디렉터리는 운영 자격(auth.json)을 담는다.\n'
 ...
 *  그건 자격이 남의 레포에 박히는 것보다 낫다. 복구는 `monad config sync-test` 한 줄이다. */
...
/** 파생 루트의 트리 루트 — `treeDerivedRootFor` 가 `<tree>/.monad-test` 를 주므로 그 부모다.
 ...
 *     `.monad-test/auth.json` 을 커밋해 둔 상태라면 우리가 그 위에 운영 자격을 덮어쓰고
...
function unsafeDerivedRoot(tree: string, root: string): string | null {
  ...
  try {
    const rel = relative(tree, root) || '.monad-test';
    ...
    if (existsSync(join(root, 'config.json'))) {
      ...
      if (!sealFromGit(root)) {
        ...
        const error = `기존 우주를 봉인하지 못해 자격을 격리했다(삭제 ${q.removed.length}·잔존 ${q.left.length})`
          + ' — 복구는 `monad config sync-test <그 우주>`';
