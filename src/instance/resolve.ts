...
// `elanous where` 가 켜지면 무엇이 달라지는지 미리 보여준다.
//
...
// ⚠️ leader/nest-depth/user-config 는 **lazy require** 로 부른다 — 이 모듈을 `elanousStateRoot`
//    와 `getElanousConfigDir` 이 소비하는데, 정적 import 로 묶으면
//    resolve → nest-depth → user-config → elanous-config-dir → resolve 순환이 생긴다.
...
/** 어느 층에서 결정됐나 — `elanous where` 의 "왜" 를 설명하는 값. */
...
export interface ResolveDeps {
  ...
  depth?: number;
  prodRoot?: string;
  /** 트리 파생이 고를 테스트 루트(`<트리>/.elanous-test`). */
...
export function resolveInstance(deps: ResolveDeps = {}): InstanceResolution {
  const prodRoot = normRoot(deps.prodRoot ?? join(homedir(), '.elanous'));
  const depth = deps.depth ?? lazyDepth();

  // 1층 — 명시 플래그. 사람이 직접 말한 것이 항상 이긴다.
  if (deps.explicitFlagRoot) {
    // ⚠️ 명시 루트라고 무조건 test 가 아니다 — `--config-dir ~/.elanous` 처럼 **prod 를 명시**할
  ...
  if (stampRaw) {
    ...
    return {
      ...
      why: `부모가 물려준 ELANOUS_STATE_DIR(depth ${depth})`,
    };
  }
 ...
 *
 *  `ELANOUS_STATE_DIR` 은 env 라 전 자손에 자동 전파되는데 config-dir 은 argv 재부착 **7곳**에서만
 ...
 *  `effectiveInstanceRoot()` 하나이며, `elanousStateRoot`/`getElanousConfigDir` 이 그것을 공유한다. */
...
//   getElanousConfigDir → resolver → getUserConfig → (config 경로 해석) → getElanousConfigDir → …
// 원인은 "config 를 읽는 것"이 아니라 **config 의 위치가 리졸버의 결과**라는 점이었다.
// ⇒ 스위치를 `~/.elanous/config.json` **고정 경로에서 raw 로** 읽으면 순환이 사라진다.
...
/** 3층 스위치 오버라이드(테스트 전용).
 *
 * ⚠️ **왜 필요한가**: `treeDerivedTestEnabled()` 는 **개발자 머신의 `~/.elanous/config.json` 을 직접
...
/** 3층(트리 파생) 루트 계산 — `<git 트리 루트>/.elanous-test`. 트리를 못 찾으면 null.
 *
