...
// dispatchMonadAutopilotLaunch args validation + clamping + factory
...
import {
  dispatchMonadAutopilotLaunch,
  monadAutopilotLaunchRuntime,
  buildMonadAutopilotLaunchTool,
  spawnMonadAutopilotAgent,
} from './monad-autopilot-launch-runtime.js';
...
describe('dispatchMonadAutopilotLaunch · input validation', () => {
  test('missing mission → error termination + no agent spawn', async () => {
    let spawned = 0;
    const r = await dispatchMonadAutopilotLaunch(
      {},
      {
        spawnAgent: async () => {
          spawned += 1;
...
describe('dispatchMonadAutopilotLaunch · factory failure path', () => {
...
describe('dispatchMonadAutopilotLaunch · goal-file traced paths', () => {
  ...
  test('wires the default repository reader when no override is supplied', async () => {
    ...
    try {
      ...
      const outside = await dispatchMonadAutopilotLaunch(
        { mission: 'do thing', goalFile: 'GOAL-traced.txt', cwd: root },
      ...
      const present = await dispatchMonadAutopilotLaunch(
        { mission: 'do thing', goalFile: 'GOAL-traced.txt', cwd: root },
...
describe('dispatchMonadAutopilotLaunch · cap clamping', () => {
...
describe('dispatchMonadAutopilotLaunch · manager ownership', () => {
...
describe('spawnMonadAutopilotAgent · canonical acquisition', () => {
  test('routes codex-app-server through the canonical manager', async () => {
    ...
    const acquired = await spawnMonadAutopilotAgent(
      'codex-app-server',
      '/repo',
      undefined,
  ...
  test('stops an injected agent and preserves its start error', async () => {
    ...
    await expect(spawnMonadAutopilotAgent('codex-app-server', '/repo', () => agent)).rejects.toThrow('start-failed');
...
describe('monadAutopilotLaunchRuntime · ToolRuntime interface', () => {
  test('exposes id and spec', () => {
    expect(monadAutopilotLaunchRuntime.id).toBe('monad_autopilot_launch');
    expect(monadAutopilotLaunchRuntime.spec.name).toBe('monad_autopilot_launch');
  });

  // ⚠️ 2026-08-01 계약 변경([T] 교차 리뷰): goalFile 은 **선택**이다.
  //   필수로 두면 기존 MCP 호출자가 전부 깨진다. 주면 린트, 안 주면 종전대로.
  test('buildMonadAutopilotLaunchTool requires mission (goalFile 은 선택)', () => {
    const spec = buildMonadAutopilotLaunchTool();
