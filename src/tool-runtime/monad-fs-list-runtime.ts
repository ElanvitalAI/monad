...
// `monad_fs_list` MCP tool. Mirrors the daemon-side ACP method
// `monad/fs/list` (src/acp/server.ts:1960). Session-agnostic so the
...
export interface MonadFsListArgs {
...
export interface MonadFsListEntry {
  name: string;
  isDir: boolean;
...
export interface MonadFsListResult extends Record<string, unknown> {
  output: string;
  cwd: string;
  root: 'cwd' | 'obsidian';
  entries: MonadFsListEntry[];
  error?: string;
}

export function buildMonadFsListTool(): LLMToolSpec {
  return {
    name: 'monad_fs_list',
    description:
      'List entries under a directory (clamped to either the daemon cwd or the Obsidian vault). Returns {name, isDir, relPath}. Codex app-server callback via the monad-tools MCP server. Read-only.',
    parameters: {
      type: 'object',
      properties: {
        root: {
...
export async function dispatchMonadFsList(
  args: MonadFsListArgs = {},
  opts: {
    rootResolver?: (kind: FsRootKind) => string;
  } = {},
): Promise<MonadFsListResult> {
...
export const monadFsListRuntime: ToolRuntime<MonadFsListArgs, MonadFsListResult> =
  {
    id: 'monad_fs_list',
    spec: buildMonadFsListTool(),
    async run(req) {
      return dispatchMonadFsList(req);
    },
  };
