...
// dispatchMonadFsRead branches (path required · clamp escape · dir
...
import {
  dispatchMonadFsRead,
  monadFsReadRuntime,
  buildMonadFsReadTool,
} from './monad-fs-read-runtime.js';

let workdir: string;
let resolver: (kind: FsRootKind) => string;

beforeEach(() => {
  workdir = mkdtempSync(join(tmpdir(), 'monad-fs-read-test-'));
...
describe('dispatchMonadFsRead · input validation', () => {
  test('missing path → path-required error', async () => {
    const r = await dispatchMonadFsRead({}, { rootResolver: resolver });
    expect(r.error).toBe('path-required');
  });

  test('empty path → path-required error', async () => {
    const r = await dispatchMonadFsRead({ path: '' }, { rootResolver: resolver });
    expect(r.error).toBe('path-required');
  });
});

describe('dispatchMonadFsRead · clamp', () => {
  test('escape attempt → path-escapes-root error', async () => {
    const r = await dispatchMonadFsRead(
...
describe('dispatchMonadFsRead · text content', () => {
  test('returns content (utf8) for .md', async () => {
    const p = join(workdir, 'note.md');
    writeFileSync(p, '# Heading\n\nThis is a note.');
    const r = await dispatchMonadFsRead({ path: p }, { rootResolver: resolver });
...
describe('dispatchMonadFsRead · binary content', () => {
...
describe('dispatchMonadFsRead · maxBytes bounds', () => {
...
describe('monadFsReadRuntime · ToolRuntime interface', () => {
  test('exposes id and spec', () => {
    expect(monadFsReadRuntime.id).toBe('monad_fs_read');
    expect(monadFsReadRuntime.spec.name).toBe('monad_fs_read');
  });

  test('buildMonadFsReadTool requires path in spec', () => {
    const spec = buildMonadFsReadTool();
