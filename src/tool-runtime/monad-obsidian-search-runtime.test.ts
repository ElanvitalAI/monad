...
// parseRgMatchStream pure parser + dispatchMonadObsidianSearch
...
import {
  parseRgMatchStream,
  dispatchMonadObsidianSearch,
  monadObsidianSearchRuntime,
  buildMonadObsidianSearchTool,
} from './monad-obsidian-search-runtime.js';
...
describe('dispatchMonadObsidianSearch · input validation', () => {
  test('empty query → query-required error', async () => {
    const r = await dispatchMonadObsidianSearch({}, { vaultRoot: VAULT_ROOT });
    expect(r.error).toBe('query-required');
    expect(r.matches).toEqual([]);
  });

  test('whitespace-only query → query-required error', async () => {
    const r = await dispatchMonadObsidianSearch({ query: '   ' }, { vaultRoot: VAULT_ROOT });
    expect(r.error).toBe('query-required');
  });
});

describe('dispatchMonadObsidianSearch · spawn integration (mocked)', () => {
  test('success path returns matches + summary', async () => {
    const stdout = [
      rgMatch('Setup/oauth.md', 42, 'OAuth setup\n'),
      rgMatch('Daily/2026-05-15.md', 7, 'Reviewed OAuth flow\n'),
    ].join('\n');
    const r = await dispatchMonadObsidianSearch(
      { query: 'OAuth' },
      {
...
describe('monadObsidianSearchRuntime · ToolRuntime interface', () => {
  test('exposes id and spec', () => {
    expect(monadObsidianSearchRuntime.id).toBe('monad_obsidian_search');
    expect(monadObsidianSearchRuntime.spec.name).toBe('monad_obsidian_search');
  });

  test('buildMonadObsidianSearchTool returns valid LLMToolSpec with required query', () => {
    const spec = buildMonadObsidianSearchTool();
    expect(spec.name).toBe('monad_obsidian_search');
