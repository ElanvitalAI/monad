...
// `monad_autopilot_launch` MCP tool. Codex turn calls this to fire a
// monad autopilot mission against a chosen backend (claude / gemini /
...
// stalled tool. Heavy missions should go through `monad autopilot run`
...
export interface MonadAutopilotLaunchArgs {
...
export interface MonadAutopilotLaunchResult extends Record<string, unknown> {
  output: string;
  ok: boolean;
...
export function buildMonadAutopilotLaunchTool(): LLMToolSpec {
  return {
    name: 'monad_autopilot_launch',
    description:
      'Run a monad autopilot mission to completion (sync MVP). Spawns an ACP agent (codex default · JDG-S9), drives the AutopilotLoopDriver, and returns aggregated text + termination. Strict caps (default 1 iteration / 120s wall-clock) — heavy missions belong in `monad autopilot run`. MCP callback only.',
    parameters: {
      type: 'object',
      required: ['mission'],
      properties: {
        ...
        backend: {
          type: 'string',
          description: "Backend id (default: DEFAULT_REVIEW_BACKEND = codex · JDG-S9). Valid: monad-builtin, codex-app-server, claude, gemini, grok.",
        },
        maxIterations: {
          type: 'integer',
...
export async function dispatchMonadAutopilotLaunch(
  args: MonadAutopilotLaunchArgs = {},
  opts: DispatchOpts = {},
): Promise<MonadAutopilotLaunchResult> {
  const t0 = Date.now();
  const mission =
  ...
  //   ⊕ `#6348`(monad dev)이 `--allow-no-evidence` 탈출구를 둔 것과 같은 형태다.
  ...
  try {
    agent = opts.spawnAgent
      ...
      : await spawnMonadAutopilotAgent(backend, cwd, undefined, agentManager);
...
export async function spawnMonadAutopilotAgent(
  backend: string,
  cwd: string,
  ...
  const agentOpts: Omit<AcpAgentOpts, 'backendId'> = {
    cwd,
    log: (message) => debug.log('monad-autopilot-launch', 'agent-log', { backend, message }),
  };
  if (createAgent) {
...
type MonadAutopilotLaunchDispatcher = (
  args: MonadAutopilotLaunchArgs,
) => Promise<MonadAutopilotLaunchResult>;

let runtimeDispatcher: MonadAutopilotLaunchDispatcher = dispatchMonadAutopilotLaunch;

/** Test seam for the default runtime's harness dispatcher. */
export function setMonadAutopilotLaunchRuntimeDispatcherForTest(
  dispatcher?: MonadAutopilotLaunchDispatcher,
): void {
  runtimeDispatcher = dispatcher ?? dispatchMonadAutopilotLaunch;
}

export const monadAutopilotLaunchRuntime: ToolRuntime<
  MonadAutopilotLaunchArgs,
  MonadAutopilotLaunchResult
> = {
  id: 'monad_autopilot_launch',
  spec: buildMonadAutopilotLaunchTool(),
  async run(req) {
