...
// `monad_obsidian_info` MCP tool. Surfaces whether an Obsidian vault
// is resolvable + its absolute path + the resolution source (config /
// env / backup / default / discovery / none). Session-agnostic mirror
// of the ACP method `monad/obsidian/info` (src/acp/server.ts:2119).
//
// Codex turn uses this to gate other monad_obsidian_* calls — if
...
export interface MonadObsidianInfoResult extends Record<string, unknown> {
...
export function buildMonadObsidianInfoTool(): LLMToolSpec {
  return {
    name: 'monad_obsidian_info',
    description:
      'Report Obsidian vault availability + absolute path + resolution source. Use to gate other monad_obsidian_* calls. Codex app-server callback via the monad-tools MCP server. Read-only · no args.',
    parameters: {
      type: 'object',
      properties: {},
...
export function dispatchMonadObsidianInfo(
  opts: { resolver?: () => { root: string; available: boolean; source: string } } = {},
): MonadObsidianInfoResult {
...
export const monadObsidianInfoRuntime: ToolRuntime<
  Record<string, unknown>,
  MonadObsidianInfoResult
> = {
  id: 'monad_obsidian_info',
  spec: buildMonadObsidianInfoTool(),
  async run() {
    return dispatchMonadObsidianInfo();
  },
};
