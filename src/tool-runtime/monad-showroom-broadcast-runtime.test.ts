...
// dispatchMonadShowroomBroadcast args parsing + broadcaster injection.

import { describe, test, expect } from 'bun:test';
import {
  dispatchMonadShowroomBroadcast,
  monadShowroomBroadcastRuntime,
  buildMonadShowroomBroadcastTool,
} from './monad-showroom-broadcast-runtime.js';
...
describe('dispatchMonadShowroomBroadcast · input validation', () => {
  test('missing prompt → output indicates required + empty targets', async () => {
    const calls: BroadcastInvocation[] = [];
    const r = await dispatchMonadShowroomBroadcast(
      {},
...
describe('dispatchMonadShowroomBroadcast · happy path', () => {
  ...
  test('honors custom backends + cwd + timeout', async () => {
    const calls: BroadcastInvocation[] = [];
    await dispatchMonadShowroomBroadcast(
      {
        prompt: 'x',
...
describe('dispatchMonadShowroomBroadcast · backend filtering', () => {
...
describe('monadShowroomBroadcastRuntime · ToolRuntime interface', () => {
  test('exposes id and spec', () => {
    expect(monadShowroomBroadcastRuntime.id).toBe('monad_showroom_broadcast');
    expect(monadShowroomBroadcastRuntime.spec.name).toBe('monad_showroom_broadcast');
  });

  test('buildMonadShowroomBroadcastTool requires prompt + lists default backends', () => {
    const spec = buildMonadShowroomBroadcastTool();
