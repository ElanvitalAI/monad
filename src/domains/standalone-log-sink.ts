...
// `monad logs` 로 조회 불가(= 관측 안 한 것). 이 헬퍼가 run-mission 이 쓰던 3줄 블록을 캡슐화해
...
export async function registerStandaloneLogSink(surface: string): Promise<boolean> {
  try {
    ...
    // LF7 — 격리 인스턴스면 레지스트리 등록 → `monad logs --instance <name>`/`--all`/`instances`
    // 로 발견 가능(2026-07-16 discord-test 러너 실측). ⚠️ prod 루트는 스킵 — 크론 등 prod
    // 프로세스가 prod 데몬 레지스트리 항목을 덮지 않게(오염 방지·stateDir 키 upsert).
    //
    // ⚠️ 판정 축 정정(2026-07-27) — 종전엔 `MONAD_STATE_DIR` 의 **존재 여부**로 게이팅했다.
    //   3층(트리 파생 test) 스위치가 켜진 뒤 워크트리 self-dev 자식은 **env 없이 리졸버로**
    //   test 우주를 정하므로, `.monad-test/logs/logs.db` 에 로그를 쓰면서도 이 가드에 걸려
    ...
    await (async () => {
      try {
        ...
        //   종전엔 `basename === '.monad-test'` 로만 봐서 `~/.monad/telegram-test` 같은
        ...
        //   repoPath 부착만 `<repo>/.monad-test` 관례에 한정한다(그때만 의미가 있다).
        const isRepoTest = pathMod.basename(stateDir) === '.monad-test';
