...
//
// monad 의 "기억" DB(에피소드 surface_events · 시맨틱 knowledge)는 종전 메인 state dir
// `~/.monad/conatus/`(시그널·미션·스케줄·로그가 다 뒤섞인 junk drawer)에 얹혀 있었다.
// 이들을 **전용 `~/.monad/memory/` 네임스페이스**로 모아 관리 표면을 통일한다.
//
// 일반화: DB 마다 경로/마이그레이션을 하드코딩(surface-events 인라인 선례)하지 않고, 이 한
// 헬퍼로 등록만 하면 (1) 정본 경로 해석 (2) conatus→memory 자가치유 이전을 공유한다.
//
//   전부 scoped — MONAD_STATE_DIR 격리 인스턴스는 각자 파일(인스턴스별 self-log). knowledge 도
...
/** managed 메모리/지식 DB 의 정본 경로. MONAD_STATE_DIR 격리 인스턴스는 자기 파일, 아니면
 *  `~/.monad/memory/`. (global opt-in 은퇴 — 전부 scoped·격리 불변식.) */
export function memoryDbPath(name: string): string {
  ...
  return join(homedir(), '.monad', 'memory', name);
}

/** legacy `~/.monad/conatus/<name>` → 정본 memory/ 자가치유 이전. 신 경로 부재 + legacy 존재
...
export function migrateLegacyMemoryDb(name: string): void {
  ...
  // scoped 격리(MONAD_STATE_DIR)엔 conatus legacy 가 없다 — prod(미설정)만 이전.
  if (process.env.MONAD_STATE_DIR?.trim()) return;
  if (existsSync(target)) return;
  const legacy = join(homedir(), '.monad', 'conatus', name);
