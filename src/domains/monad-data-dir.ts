...
//
// 왜 생겼나: `~/.monad/conatus/**` 경로가 **69지점에 하드코딩**돼 있었고, 올바른
...
/** 데이터 루트 (중립 이름).
 *
 *  우선순위: `CONATUS_DATA_DIR`(기존 노브·파리티 격리) → `MONAD_STATE_DIR`/conatus →
 *  **NODE_ENV=test 리다이렉트** → `~/.monad/conatus`.
 *
 *  ⚠️ 테스트 리다이렉트가 `~/.monad` 기본값보다 **앞**에 있다. 격리를 잊은 테스트가
...
export function monadDataDir(): string {
  ...
  return join(homedir(), '.monad', 'conatus');
}
...
function testFallbackDir(): string {
  ...
  testFallbackRoot = join(tmpdir(), `monad-test-conatus-${process.pid}`);
  ...
  try {
    debug.log('conatus.data-dir', 'test-root-redirect', {
      ...
      why: 'NODE_ENV=test without CONATUS_DATA_DIR/MONAD_STATE_DIR',
    });
...
export function monadDataPath(...parts: string[]): string {
  return join(monadDataDir(), ...parts);
}

/** 호환 export — 옛 이름. 디스크 경로·해석 순서는 `monadDataDir` 과 같다. */
export function conatusDataDir(): string {
  return monadDataDir();
}

/** 호환 export — 옛 이름. `monadDataPath` 와 같은 문자열. */
export function conatusPath(...parts: string[]): string {
  return monadDataPath(...parts);
}
