...
//   elanous(3곳 각자 하드코딩)·스킬 전반에 박혀 폴더 rename/정리 시 mass 파손 landmine 이었다. 이제
//   elanous 는 이 한 곳으로 수렴하고, 해석 순서 = env `CONATUS_DIR` → stable 심링크 `~/source/conatus`
//   → dated 폴백(하위호환). 폴더명이 바뀌어도 심링크만 re-point 하면 elanous 무영향. [[feedback_config_over_env]]
...
/** CONATUS 소스 루트(투자 데이터·screener 스크립트·.env) — elanous 단일 출처. */
export const CONATUS_DIR = resolveConatusDir();

/** creds .env 경로 해석 — elanous 소유 우선(Conatus 소스 dir 의존 종식·2026-07-22).
 *  순서 = env `CONATUS_ENV` override → `~/.elanous/conatus/.env`(elanous 소유·데몬/크론 무관 읽힘)
 *  → `CONATUS_DIR/.env`(하위호환 폴백). creds 를 elanous 소유 위치에 두면 Conatus 소스 dir 을
 *  옮기거나 archive 해도 elanous 자격 로드 무영향. */
function conatusEnvPath(): string | null {
  const candidates = [
    process.env.CONATUS_ENV || '',
    join(homedir(), '.elanous', 'conatus', '.env'),
    join(CONATUS_DIR, '.env'),
  ].filter(Boolean);
  return candidates.find(p => existsSync(p)) ?? null;
}

/** CONATUS creds(.env·토스 자격 등)를 env 객체로 로드. elanous 소유 `~/.elanous/conatus/.env`
