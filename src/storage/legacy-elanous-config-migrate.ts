...
//   Once-per-process migration of ~/.config/elanous/config.json (Path A ·
//   19 root keys) into ~/.elanous/config.json (Path B · NEXUS schema).
//
...
//   - ~/.config/elanous/config.json absent → skip
...
export function migrateLegacyXdgUserConfig(): void {
  ...
  const oldPath = join(home, '.config', 'elanous', 'config.json');
  const newPath = join(home, '.elanous', 'config.json');
...
export function __resetElanousConfigMigrateForTests(): void {
  migrated = false;
}
