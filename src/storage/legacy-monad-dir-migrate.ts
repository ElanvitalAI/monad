...
//   one-time migrate ~/.config/monad/<subdir>/ → ~/.monad/<subdir>/
//
// Used by storage modules (policy, budget) whose DEFAULT_STORAGE_DIR
// flipped from XDG to ~/.monad/. SQLite-aware (multi-file copy that
...
/** Resolve the home directory. Tests inject via `MONAD_TEST_HOME` so they
 *  cannot accidentally migrate the user's actual ~/.config/monad/ — Bun's
...
export function migrateLegacyXdgSubdir(subdir: string): void {
  ...
  const oldDir = join(home, '.config', 'monad', subdir);
  const newDir = join(home, '.monad', subdir);
 ...
 *  ~/.config/monad/<filename> → ~/.monad/<filename> once-per-process,
...
export function migrateLegacyXdgFile(filename: string, mode: number = 0o600): void {
  migrateLegacyHomeFile({
    legacyHomeRel: join('.config', 'monad', filename),
    monadRel: filename,
    mode,
  });
}
 ...
 *  don't fit the `~/.config/monad/<name>` shape `migrateLegacyXdgFile`
...
export interface MigrateLegacyHomeFileOpts {
  ...
  /** Path relative to ~/.monad/ for the new location, e.g. 'hints.json'. */
  monadRel: string;
  mode?: number;
}
...
export function migrateLegacyHomeFile(opts: MigrateLegacyHomeFileOpts): void {
  ...
  const home = (() => {
    const t = process.env.MONAD_TEST_HOME?.trim();
    return t ? t : homedir();
  })();
  const oldPath = join(home, opts.legacyHomeRel);
  const newPath = join(home, '.monad', opts.monadRel);
 ...
 *  legacy dirs (e.g. `~/.monad-agent/audit/` → `~/.monad/audit/`). */
...
export function migrateLegacyHomeDir(opts: MigrateLegacyHomeDirOpts): void {
  ...
  const newDir = join(home, '.monad', opts.monadRel);
