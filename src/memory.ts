 ...
 *  Matches the rest of monad (SKILL.md parsing). */
...
/** Rebuild ~/.local/share/monad/memory/MEMORY.md. Format mirrors the
...
export function rebuildIndex(root: string = memoryRoot()): void {
  ...
  const lines: string[] = [];
  lines.push('# monad memory index');
  lines.push('');
  lines.push(`<!-- auto-generated by monad — edit individual .md files instead -->`);
  lines.push('');
...
export function makeMemoryJudge(model?: string): (prompt: string) => Promise<string> {
  const m = model || process.env.MONAD_MEMORY_JUDGE_MODEL || budgetModel();
