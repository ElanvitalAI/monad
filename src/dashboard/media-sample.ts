function buildDashboardMediaSampleImageText(): string {
  const svg = [
    ...
    `<text x="84" y="156" fill="#f4f8fb" font-size="46" font-family="Menlo, monospace">Elanous media sink sample</text>`,
  ...
  return `![Elanous media sink sample](data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)})`;
}
...
function buildDashboardMediaSampleVideoText(url?: string): string {
  ...
  return `[Elanous media sink sample video](${target})`;
}
