...
// 감독 루프·원장이 수정 없이 따라온다(RFC-monad-on-docker-and-kubernetes-isolation-ladder · ROADMAP C2).
// Pod 안에서는 같은 명령 `monad self implement "<feature>" --json …` 을 치고, 마지막 줄 JSON 을 Job 로그로 읽는다.
//
...
//   · 이미지는 `docker/harness/Dockerfile`(monad 설치 ⊕ 정적 codex·짝·rg·gh) — 이 모듈은 이미지를 «만들지» 않는다.
...
export interface PodSpawnOptions {
  /** 이미지 판(라벨 monad.commit) — 주입(시험·감독기가 이미 잰 값). 없으면 docker 로 잰다. */
  ...
  /** Pod 에 그대로 넣을 «비밀 아닌» env — 벤치 팔의 `MONAD_LLM_PROVIDER`·`MONAD_LLM_MODEL`·`MONAD_ARM_ID`. */
...
/** 호스트 계정 → refresh 없는 사본(monad 저장소 ⊕ codex auth.json) ⊕ gh 토큰. */
export function hostCredentials(account: string, storePath: string = authStorePath(), ghToken: () => string = defaultGhToken): { monadAuth: string; codexAuth: string; ghToken: string } {
  // 경로는 해석기로(격리 게이트 · 2026-09-25) — 손으로 `~/.monad/auth.json` 을 조립하면 test↔prod 격리가 새는 자리가 된다.
  ...
  const monadAuth = JSON.stringify({
  ...
  return { monadAuth, codexAuth, ghToken: ghToken() };
}
...
for i in $(seq 1 60); do
  if curl -s -m 1 -o /dev/null http://host.orb.internal:31415/health || curl -s -m 1 -o /dev/null http://core.monad-prod:8080/; then ok=0; else ok=$((ok+1)); fi
...
export function podJobManifest(o: { name: string; namespace: string; image: string; repoUrl: string; args: readonly string[]; passEnv: readonly string[]; deadlineSeconds: number; runId?: string; armEnv?: Readonly<Record<string, string>>; hostId?: string; imageCommit?: string | null; skillEnvs?: readonly string[]; memoryLimit?: string }): Record<string, unknown> {
  ...
  const script = [
    ...
    `monad self implement "$(cat /creds/feature)" --json ${quoted} > /tmp/si.out 2>&1; rc=$?`,
    'cat /tmp/si.out',
    '[ -f scripts/usage-rollup.ts ] && bun scripts/usage-rollup.ts --since 12h || echo "MONAD_USAGE_ROLLUP {\"measured\":false,\"reason\":\"no rollup script\"}"',
    'tail -n 1 /tmp/si.out',
    'exit $rc',
  ].join('\n');
  return {
    apiVersion: 'batch/v1',
    kind: 'Job',
    metadata: { name: o.name, namespace: o.namespace, labels: { 'monad.substrate': 'pod', 'monad.job': o.name } },
    spec: {
      backoffLimit: 0,
      ttlSecondsAfterFinished: 7200,
      activeDeadlineSeconds: o.deadlineSeconds,
      template: {
        // ⭐ Pod 라벨 — local 팔만 `monad.egress/local-llm` 을 단다. docker/h1/policy-local-llm.yaml 이 그 라벨에만 호스트 LLM 포트 «하나»를 연다.
        metadata: { labels: { 'monad.job': o.name, ...(o.armEnv?.MONAD_LLM_PROVIDER === 'local' ? { 'monad.egress/local-llm': 'true' } : {}) } },
        spec: {
          restartPolicy: 'Never',
          ...
          containers: [{
            ...
            env: [
              ...
              ...(o.runId ? [{ name: 'MONAD_RUN_ID', value: o.runId }] : []),
              { name: 'MONAD_SUBSTRATE', value: 'pod' },
              // ⭐ 런 출처(🅣 RFC run-origin §A3 · #20457/#20468 칸 이름) — Pod 는 자기 monad_id 를 쓰지 않고 «띄운 감독기»의 hostId 를 물려받는다.
              { name: 'MONAD_POD_NAME', valueFrom: { fieldRef: { fieldPath: 'metadata.name' } } },
              { name: 'MONAD_NODE_NAME', valueFrom: { fieldRef: { fieldPath: 'spec.nodeName' } } },
              { name: 'MONAD_POD_NAMESPACE', valueFrom: { fieldRef: { fieldPath: 'metadata.namespace' } } },
              ...(o.hostId ? [{ name: 'MONAD_HOST_ID', value: o.hostId }] : []),
              ...(o.imageCommit ? [{ name: 'MONAD_IMAGE_COMMIT', value: o.imageCommit }] : []),
...
/** Pod 가 낸 `MONAD_USAGE_ROLLUP` 한 줄 → 호스트 logs.db 에 `llm-usage`(site=`pod-rollup:<site>`)로 재방출(RFC F2).
...
export function reemitPodUsage(logs: string, job: string, log: (category: string, event: string, data: Record<string, unknown>) => void = (c, e, d) => debug.log(c, e, d), reprice?: PodRowReprice): number {
  const line = logs.split('\n').reverse().find((l) => l.startsWith('MONAD_USAGE_ROLLUP '));
  ...
  try { parsed = JSON.parse(line.slice('MONAD_USAGE_ROLLUP '.length)); } catch { log('self-implement.pod', 'usage-rollup-unparsable', { job }); return 0; }
...
export function podSubstrateReady(kubectl: Kubectl = defaultKubectl, image = 'monad-harness:local'): { ok: boolean; reason: string } {
  const ctx = kubectl(['config', 'current-context']);
  if (ctx.status !== 0) return { ok: false, reason: 'kubectl context 없음 — k3d cluster create monad-h1 --no-lb' };
  const ns = kubectl(['get', 'ns', 'monad-test']);
  if (ns.status !== 0) return { ok: false, reason: 'monad-test 네임스페이스 없음 — kubectl apply -f docker/h1/base.yaml -f docker/h1/policy-internet.yaml' };
  void image;
 ...
 *    승급하고, 그 env 가 `MONAD_LLM_PROVIDER` 를 이긴다(`user-config.ts` escalate → runtime → config).
...
export function benchArmEnv(arm: BenchArm): Record<string, string> {
  const env: Record<string, string> = { MONAD_ARM_ID: `pod/${arm.id}`, MONAD_LLM_PROVIDER: arm.provider };
  if (arm.model) env.MONAD_LLM_MODEL = arm.model;
  // 🖥️ local 팔 — 호스트의 OpenAI 호환 서버(LM Studio 기본 1234). Pod 에서 호스트는 `host.orb.internal`(OrbStack).
  //   ⛔ 사설망 차단 정책이 기본이라 라벨(`monad.egress/local-llm`) ⊕ policy-local-llm.yaml 이 그 포트 하나만 연다.
  if (arm.provider === 'local') env.LOCAL_LLM_URL = process.env.MONAD_BENCH_LOCAL_LLM_URL?.trim() || 'http://host.orb.internal:1234/v1';
  for (const tier of ['SOL', 'OPUS']) {
    env[`MONAD_SELFDEV_${tier}_PROVIDER`] = arm.provider;
    if (arm.model) env[`MONAD_SELFDEV_${tier}_MODEL`] = arm.model;
  }
  return env;
}

// ── 이미지 판 (BACKLOG E6) ────────────────────────────────────────────────────────────
/** Pod 안의 `monad` 는 이미지에 구운 설치본이다 — clone 한 main 이 아니다.
 ...
 *  ⇒ 라벨 `monad.commit`(docker/harness/build.sh)과 이 트리 HEAD 를 대조한다. */
...
export function podImageFreshness(deps: {
  ...
  const img = run('docker', ['image', 'inspect', image, '--format', '{{index .Config.Labels "monad.commit"}}']);
