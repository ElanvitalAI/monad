...
//
// 외부 도구(Claude Code·Codex 세션·스킬·git hook)가 elanous 데몬에 "무엇을 구현/
// 변경했나(+문서)"를 주입하는 진입점. CLI `elanous self log` 의 HTTP 대응 —
...
//   summary 필수. docPath 있으면 문서 벡터 인제스트(fail-soft). domain='elanous' 고정.
...
/** POST /v1/self-event — 외부 구현/변경 이벤트를 elanous 자기인지 기억에 주입. */
