...
//
// Wraps `monad serve` as a supervisor-managed tab. Default policy per
// HANDOFF §2.2 + D-1/D-3/D-4:
//   - command:        [<monad-bin>, 'serve']  (foreground; supervisor backgrounds it)
//   - health:         socket connect to monadDaemonSocketPath() every 10s
//                     (stale after 30s → unhealthy)
//   - restart:        on-crash · backoff [5s, 15s, 60s] · maxPerHour 5
//   - external:       if `~/.monad/monad.pid` already holds an alive pid
...
// is intentionally NOT passed here — `monad serve` itself is the daemon.
...
export interface DaemonTabOpts {
  ...
  label?: string;
  /** Argv override for the spawn. Defaults to `[<monad>, 'serve']`. */
  command?: string[];
  /** cwd override. Defaults to process.cwd(). */
  cwd?: string;
  /** Extra env pairs merged over process.env at spawn time. */
  env?: Record<string, string>;
  /** Override socket path probe target (default = monadDaemonSocketPath()). */
  socketPath?: string;
}

function defaultMonadCommand(): string[] {
  // process.argv[1] points at the monad bundle when invoked via `monad nexus`.
  // Falls back to PATH lookup of `monad` for tests / unusual entrypoints.
  const bin = process.argv[1];
  return [bin && bin.length > 0 ? bin : 'monad', 'serve'];
}
...
export function createDaemonTabSpec(opts: DaemonTabOpts = {}): TabSpec {
  ...
  const command = opts.command ?? defaultMonadCommand();
  return {
    id,
    kind: DAEMON_KIND,
    label: opts.label ?? id,
    ...
    health: {
      ...
      timeoutMs: 1_500,
      staleAfterMs: 30_000,
      spec: { path: opts.socketPath ?? monadDaemonSocketPath() },
    },
    restart: {
      policy: 'on-crash',
    ...
    meta: {
      socketPath: opts.socketPath ?? monadDaemonSocketPath(),
...
export interface DetectExternalDaemonOpts {
  ...
  readLock?: () => MonadDaemonLockMeta | null;
  isAlive?: (meta: MonadDaemonLockMeta) => boolean;
}
...
/** Inspect the shared `~/.monad/monad.pid` lock. If an alive pid is
...
export function detectExternalDaemon(opts: DetectExternalDaemonOpts): DetectExternalDaemonResult {
  ...
  const readLock = opts.readLock ?? readMonadDaemonLock;
  const isAlive = opts.isAlive ?? isAliveMonadDaemonLock;
