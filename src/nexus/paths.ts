...
//   2. `process.env.MONAD_NEXUS_DIR` — LEGACY fallback (restored
...
//      `~/.monad/nexus/.lock`, repeatedly stopping the production
...
//      `--config-dir` instead.
//   3. `<getMonadConfigDir()>/nexus/` — default. `--config-dir <path>`
 ...
 *  under `<repo>/.monad-test/nexus/` while config.json / secrets stay
 *  in `~/.monad/`. Pass `null` to clear (test cleanup). */
...
export function nexusRootDir(): string {
  ...
  // An EXPLICIT `--config-dir` / setMonadConfigDir wins over the legacy
  // env (config-dir-unify semantics preserved for its adopters).
  const cfgOverride = getMonadConfigDirOverride();
  ...
  const legacy = process.env.MONAD_NEXUS_DIR;
  if (legacy && legacy.trim()) return legacy.trim();
  return joinPath(getMonadConfigDir(), 'nexus');
}
...
/** P-2D.1 — lock file for the detached `monad nexus pwa dev --bg`
