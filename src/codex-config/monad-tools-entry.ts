...
//   2. `[mcp_servers.monad-tools]` pointing at `monad mcp serve` so
//      codex spawns our stdio MCP server and discovers the 5
//      monad_* tools (H1·5a-e).
//
// Pure projector — no I/O, no fs access. Tests assert the exact
// emitted shape so we can detect accidental drift.

export interface MonadToolsEntryOpts {
  /** Absolute path to the monad CLI binary. Defaults to `'monad'`
   ...
   *  inject `MONAD_DAEMON_SOCKET` etc when needed. */
...
const DEFAULT_OPTS: Required<
  Omit<MonadToolsEntryOpts, 'env' | 'defaultPermissions'>
...
export function renderMonadToolsEntry(opts: MonadToolsEntryOpts = {}): string {
  const merged = {
    ...DEFAULT_OPTS,
    ...opts,
  ...
  lines.push('[mcp_servers.monad-tools]');
