...
const savedState = process.env.ELANOUS_STATE_DIR;
afterEach(() => {
  resetElanousConfigDir();
  if (savedState === undefined) delete process.env.ELANOUS_STATE_DIR;
  else process.env.ELANOUS_STATE_DIR = savedState;
});

describe('missionCliScopeError', () => {
  test('ELANOUS_STATE_DIR 미설정 → 통과(운영 디폴트)', () => {
    delete process.env.ELANOUS_STATE_DIR;
    expect(missionCliScopeError()).toBeNull();
  });

  test('ELANOUS_STATE_DIR == config-dir → 통과(데몬이 둘 다 맞춘 정합)', () => {
    process.env.ELANOUS_STATE_DIR = '/repo/.elanous-test';
    setElanousConfigDir('/repo/.elanous-test');
    expect(missionCliScopeError()).toBeNull();
  });

  test('⚠️ ELANOUS_STATE_DIR=test 인데 config-dir=운영 → 거부(내가 당한 그 사고)', () => {
    process.env.ELANOUS_STATE_DIR = '/repo/.elanous-test';
    ...
    expect(err).toContain('--config-dir /repo/.elanous-test');
  });
