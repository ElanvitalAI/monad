...
function seed(dbPath: string): void {
  ...
  try {
    ...
    upsertBuild(db, { ...base, buildId: makeBuildId(PHASE, 1), attemptSeq: 1, backend: 'monad-self:gpt-5.6-terra', status: 'gate-failed', maxTurns: 150, gateResult: 'bun test 3 fail' });
    upsertBuild(db, { ...base, buildId: makeBuildId(PHASE, 2), attemptSeq: 2, backend: 'monad-self:gpt-5.6-terra', status: 'gate-failed', maxTurns: 1000 });
...
describe('attemptsForPhase — 실측 시도 트레일', () => {
  test('attemptSeq 순 매핑 · running 제외 · gate 매핑(gate-failed/error) · 발췌 보존', () => {
    ...
    expect(attempts[0]).toEqual({ backend: 'monad-self:gpt-5.6-terra', maxTurns: 150, gateResult: 'gate-failed', gateOutputExcerpt: 'bun test 3 fail' });
