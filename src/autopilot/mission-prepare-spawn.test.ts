...
describe('missionPrepareLogPath', () => {
  it('미션 id hash 세그먼트로 미션별 경로', () => {
    ...
    // ⚠️ 뿌리를 **하드코딩하지 않는다**(2026-07-27 정정) — 종전엔 `.monad/conatus` 를 단언해
    //    *"이 프로세스는 운영 우주"* 를 암묵 가정했다. `bunfig` preload(#c2e895a · 2026-07-25)가
    //    `MONAD_STATE_DIR` 미설정 시 전역 격리 tmp 를 강제하면서 그 가정이 깨져 계속 빨간 채였다.
    //    시험 대상은 **뿌리가 어디냐가 아니라 그 아래 conatus 세그먼트**이므로 리졸버에 맞춰 단언한다.
    expect(p).toContain(join(monadStateRoot(), 'conatus'));
  });
  ...
  // ★ 인스턴스 스코프(ISO·2026-07-14) — MONAD_STATE_DIR 존중(homedir 고정 은퇴).
  it('MONAD_STATE_DIR 설정 시 test 루트 아래로(로그 누수 방지)', () => {
    const saved = process.env.MONAD_STATE_DIR;
    try {
      process.env.MONAD_STATE_DIR = '/tmp/monad-test-root';
      const p = missionPrepareLogPath('apm_x_abc123');
      expect(p).toBe('/tmp/monad-test-root/conatus/mission-prepare-abc123.log');
    } finally {
      if (saved === undefined) delete process.env.MONAD_STATE_DIR;
      else process.env.MONAD_STATE_DIR = saved;
    }
  });
});
