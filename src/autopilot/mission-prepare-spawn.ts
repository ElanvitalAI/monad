 ...
 *  ★ 인스턴스 스코프(ISO·2026-07-14) — homedir 고정 은퇴. `ELANOUS_STATE_DIR` 존중 → 격리 테스트
 *  데몬의 준비 로그는 test 루트에, 운영은 ~/.elanous 에(로그 누수 방지·surface-events 동형). */
export function missionPrepareLogPath(missionId: string): string {
  ...
  return join(elanousStateRoot(), 'conatus', `mission-prepare-${seg}.log`);
}
...
export function spawnMissionPrepare(
  ...
  // ★ 인스턴스 스코프 전파(ISO·2026-07-14) — config-dir 는 setElanousConfigDir in-process
  // override 라 env 로 상속 안 됨(config-dir-unify: env 미러 제거). bg-launch 처럼 argv 로
  // 재전달해야 자식 se-mission-prepare 가 같은 tasks.db/config(격리 테스트면 .elanous-test)를
  // 연다. 미전파 시 자식이 운영 스토어에서 미션을 찾다 즉사(대표 발견). ELANOUS_STATE_DIR
  ...
  args.push('--config-dir', getElanousConfigDir());
