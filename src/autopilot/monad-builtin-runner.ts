// src/autopilot/monad-builtin-runner.ts
//
// ROADMAP-monad-builtin-autopilot-cascade §MB-3.
//
// `LlmTurnRunner` 의 두 번째 구현 — ACP CLI sub-process 가 아니라
// NEXUS in-process LLM rotation 위 동작. `runCoreTurn` 을 wrap 해서
// monad 의 native tool registry · permission · rotation · in-process
...
export interface MonadBuiltinTurnRunnerOptions {
 ...
 */
export class MonadBuiltinTurnRunner implements LlmTurnRunner {
  private readonly options: MonadBuiltinTurnRunnerOptions;
  private readonly messages: LLMMessage[] = [];
  private abortController: AbortController | null = null;

  constructor(opts: MonadBuiltinTurnRunnerOptions) {
    this.options = opts;
  ...
  async prompt(
    ...
    if (debug.enabled) {
      debug.log('autopilot.monad-builtin', 'prompt.begin', {
    ...
    if (debug.enabled) {
      debug.log('autopilot.monad-builtin', 'prompt.end', {
        sessionId: this.options.sessionId,
        stopReason,
  ...
  private async resolveAutoRouteModel(userText: string): Promise<string | undefined> {
    try {
      ...
      if (debug.enabled) {
        debug.log('autopilot.monad-builtin', 'autoroute.decision', {
          sessionId: this.options.sessionId,
          tier: route.tier,
          model: route.model,
          source: route.source,
      ...
      debug.log('autopilot.monad-builtin', 'autoroute.error', {
