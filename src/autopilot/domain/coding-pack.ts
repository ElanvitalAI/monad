...
//
// executor = elanous-self 격리 worktree(승인→멀티페이즈 executor·umbrella §9)와 D2/D3 합류
 ...
 *  [[feedback_mission_fabric_llm_logic_balance_2026_07_16]]. 기존 elanous 자산 확장 같은 내부 구현 골은
...
async function assessNeed(goal: string): Promise<{ needed: boolean; reason: string }> {
  const raw = await llmJson([
    'You decide whether a coding mission for the **elanous** codebase needs EXTERNAL web research before planning.',
    'CONTEXT: elanous is a large, mature agent codebase that ALREADY integrates most common external services',
    ...
    'elanous has never used, current pricing/rates, a "latest/current" claim, or an unfamiliar external protocol.',
    'Building/extending features with capabilities elanous likely already has → needed=FALSE.',
    '', `Mission goal:\n${goal}`, '',
    'Respond with ONE JSON object, no fences: {"needed": <bool>, "reason": "<one line>"}',
  ].join('\n'), { model: process.env.ELANOUS_RESEARCH_NEED_MODEL || budgetModel(), effort: 'low' });
...
export const CODING_PACK: DomainPack = {
  ...
  decompose: {
    ...
    objectivePreamble: (goal: string) => `elanous 에 다음 미션을 구현한다: "${goal}"`,
  ...
  // elanous-self 격리 worktree phase-driver(umbrella §9·arming 게이트)가 이어받음.
