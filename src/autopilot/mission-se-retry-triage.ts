...
// 환경 제약 — monad 가 못 고치는 근본(gh 인증/네트워크/의존 부재). 골정정 또는 사람.
...
export function buildSETriagePrompt(input: SERetryTriageInput, baseline: SERetryTriageDecision): string {
  ...
  for (const a of input.attempts) {
    ...
    lines.push(`- 시도${a.attempt}: ${a.backend.replace(/^monad-self:/, '')} ${a.maxTurns}턴 -> ${a.gateStatus}${a.structural ? '(구조적)' : ''} · 사유 ${a.gateText.slice(0, 120)}${crit}`);
  }
