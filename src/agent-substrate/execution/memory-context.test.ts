...
describe('recallMemoryContext', () => {
  test('관측 장치 산출은 자식 프롬프트 컨텍스트에서 빼고 유지된 기억은 포맷한다', async () => {
    ...
    try {
      recordEvent(db, { domain: 'elanous', surface: 'test', direction: 'outbound', kind: 'utterance', text: '<task-notification>\n<summary>child memory probe one</summary>', importance: 10 });
      recordEvent(db, { domain: 'elanous', surface: 'test', direction: 'outbound', kind: 'utterance', text: 'child memory probe pty_a4c9f0: capture', importance: 10 });
      recordEvent(db, { domain: 'elanous', surface: 'test', direction: 'outbound', kind: 'impl', text: 'retained child memory probe one', summary: 'retained summary one', importance: 1 });
      recordEvent(db, { domain: 'elanous', surface: 'test', direction: 'outbound', kind: 'impl', text: 'retained child memory probe two', summary: 'retained summary two', importance: 1 });
