...
// 쓴다. 시드 allowlist 에 `MONAD_*` 가 하나도 없어서 **프로세스가 런타임에 심은 정체성이
// PTY 경계에서 통째로 소실**된다.
//
//   셸이 정의(rc 유래·ANTHROPIC_API_KEY·PATH) → 로그인 셸이 재생산 → ✅ 생존
//   프로세스가 런타임에 설정(MONAD_STATE_DIR 등) → 로그인 셸이 알 리 없음 → ❌ 소실
//
// 실측 결과: 격리된 monad(`--test`)가 띄운 PTY 안에서 monad 를 실행하면 두 축이 모두 비어
// **완전한 prod(`~/.monad`)로 떨어진다**(축 어긋남이 아니라 격리 완전 소실). 부수로
// `MONAD_NEST_DEPTH` 도 잃어 재귀 depth cap(fork-bomb 가드)이 우회된다.
//
...
const IDENTITY_ENV_KEYS = [
  ...
  'MONAD_STATE_DIR_SOURCE', // 그 뿌리가 «파생»인가 «명시»인가 — 뿌리와 «한 벌»이다
  'MONAD_CONTROL_INBOX_DIR', // 부모가 해석한 제어 inbox — 중첩 PTY도 같은 대기 자리를 본다
  'MONAD_NEST_DEPTH',       // 재귀 cap(fork-bomb 가드) — 잃으면 자식이 자기를 depth 0 으로 인식
  'MONAD_HARNESS_SPACE',    // 하니스 공간 kind ┐ harnessSpaceEnv() 가 한 벌로 세팅 — 셋이 같이
  'MONAD_HARNESS_SPACE_ID', // 하니스 공간 id   │ 가야 getHarnessSpace() 가 공간을 복원한다
  'MONAD_RUN_ID',           // run 상관관계     ┘
  'MONAD_SESSION_ID',       // 세션 귀속
  'MONAD_ORIGIN_ROOT',      // 최초 실행 시작 주체(set-once)
  'MONAD_ORIGIN_AGENT',     // 외부 에이전트 이름(set-once·external-agent일 때만)
  'MONAD_ORIGIN_SESSION',   // 외부 에이전트 세션(set-once·있을 때만)
  'MONAD_CONTROLLER',       // 현재 프로세스의 상위 PTY/run 제어자(층마다 갱신 가능)
  'MONAD_CONTROL_CHANNELS', // 자식을 조작할 수 있는 관 목록(쉼표 구분)
  'MONAD_PTY_CHAIN_ORIGIN', // PTY 실행 사슬의 tree/worktree 출발지(실행 출신과 별도 축)
] as const;

/** ⛔⭐⭐⭐ **래칫용 — `buildPtyEnv` 가 «합성»하는 키 목록**(2026-08-19 · `OBS-T118` 후속).
 *
 * 🚨 왜 있나 — 나는 `MONAD_STATE_DIR_SOURCE` 를 «합성»만 하고 ***allowlist 에 안 넣었다***.
...
export const SYNTHESIZED_IDENTITY_ENV_KEYS = [
  'MONAD_STATE_DIR',
  'MONAD_STATE_DIR_SOURCE',
] as const;

export type OriginRoot = 'external-agent' | 'monad-internal' | 'human-cli' | 'scheduler';

export function deriveChildController(env: NodeJS.ProcessEnv, pid: number): string {
  const ptyId = env.MONAD_PTY_ID?.trim();
  if (ptyId) return `pty:${ptyId}`;
  const originAgent = env.MONAD_ORIGIN_AGENT?.trim();
...
export function establishExecutionOrigin(
  ...
  if (explicitSession && env.MONAD_ORIGIN_SESSION === undefined) {
    env.MONAD_ORIGIN_SESSION = explicitSession;
  }
  const existingRoot = env.MONAD_ORIGIN_ROOT?.trim();
  if (!existingRoot) {
    const agent = env.AI_AGENT?.trim();
    if (agent) {
      env.MONAD_ORIGIN_ROOT = 'external-agent';
      env.MONAD_ORIGIN_AGENT = agent;
      const claudeSession = env.CLAUDE_CODE_SESSION_ID?.trim();
      if (claudeSession && env.MONAD_ORIGIN_SESSION === undefined) {
        env.MONAD_ORIGIN_SESSION = claudeSession;
      }
    } else {
      env.MONAD_ORIGIN_ROOT = 'human-cli';
    }
  }
  if (controller?.trim()) env.MONAD_CONTROLLER = controller;
  return (env.MONAD_ORIGIN_ROOT?.trim() || 'human-cli') as OriginRoot;
}

// ── 의도적 제외 ────────────────────────────────────────────────────────────
//
// 1. **shadow-root env**(아래) — `--config-dir`/`MONAD_STATE_DIR` 을 우회해 스토어를 인스턴스
...
// 4. **`MONAD_HARNESS_ROLE`·`MONAD_HARNESS_BOUNDARY`·`MONAD_HARNESS_DETACHED`** — 정체성이
...
const SHADOW_ROOT_ENV_KEYS = [
  'MONAD_HOME', 'MONAD_DIR', 'MONAD_NEXUS_DIR', 'MONAD_TASKS_DIR', 'MONAD_TASKS_DB',
] as const;
 ...
 *  (`MONAD_RUN_ID` · 나중에 세팅되는 `MONAD_NEST_DEPTH`)이 얼어붙는다. */
export function buildPtyEnv(
  ...
  //   ⛔ 사람이 «명시»한 값에는 안 붙는다 — 그 값은 파생 뿌리와 다를 것이기 때문이다.
  {
    ...
    if (current.kind === 'test' && identity.MONAD_STATE_DIR === current.root) {
      identity.MONAD_STATE_DIR_SOURCE = 'derived';
    }
  }
  if (!identity.MONAD_STATE_DIR) {
    const current = resolveCurrentInstance();
    if (current.kind === 'test') {
      fallbackStateDir = current.root;
      identity.MONAD_STATE_DIR = fallbackStateDir;
      // ⛔⭐⭐⭐⭐ **자식에게 「이 값이 «어디서 왔나»」를 같이 준다**(2026-08-19 · `OBS-T114`).
      //
      // 🚨 왜 필요한가 — 자식이 받는 것은 ***그냥 `MONAD_STATE_DIR` 문자열 하나***라서,
  ...
  const result = { ...captured, ...identity, ...callerEnv };
  const stateDirSource: 'env' | 'fallback' | 'absent' = !result.MONAD_STATE_DIR
    ? 'absent'
    : fallbackStateDir === result.MONAD_STATE_DIR && !Object.hasOwn(callerEnv, 'MONAD_STATE_DIR')
      ? 'fallback'
      : 'env';
