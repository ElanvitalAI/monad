// 재귀 depth cap (⑩ · 2026-07-20) — "액자 In 액자"(monad→자식 monad→손자…) 무한재귀/fork-bomb 방지.
//
// [[PLAN-unified-autonomous-agent-substrate-2026-07-20]] §6a. injector=self(monad 이 자식 monad 을
...
//
// 메커니즘: 부모가 `MONAD_NEST_DEPTH` 를 자식 env 에 실어(childNestEnv) 자식은 depth+1 로 인지.
// depth >= maxNestDepth 면 spawn 툴을 카탈로그에서 제외 + dispatch 거부(defense-in-depth).
// 상한 = config `substrate.maxNestDepth`(config-first) → env `MONAD_MAX_NEST_DEPTH` → 기본 5(액자 5중·
...
export function getNestDepth(): number {
  const raw = process.env.MONAD_NEST_DEPTH;
  const n = raw !== undefined ? Number(raw) : 0;
  return Number.isFinite(n) && n >= 0 ? Math.floor(n) : 0;
}

/** 중첩 상한. config-first(substrate.maxNestDepth) → env(MONAD_MAX_NEST_DEPTH) → 기본 5. */
export function getMaxNestDepth(): number {
  ...
  const envRaw = process.env.MONAD_MAX_NEST_DEPTH;
  if (envRaw !== undefined) {
...
export function childNestEnv(): Record<string, string> {
  return { MONAD_NEST_DEPTH: String(getNestDepth() + 1) };
}
