// H5 Phase 3 · elanous-as-child adapter.
//
// Wraps another elanous instance (spawned with `--acp-server`) as an
// `EmbodiedAgentSession` whose only transport is `kind:'acp'`. This
// enables "elanous calling elanous" — a parent agent delegates a task to
// a sub-elanous which itself can route to codex/claude/gemini/etc.
//
// Recursion is capped via an env-configurable depth limit (default 3)
// propagated through the `ELANOUS_AGENT_DEPTH` env var. Each child bumps
...
// directly. The sub-elanous's own dashboard doesn't render (because
...
export const ELANOUS_AGENT_DEPTH_ENV = 'ELANOUS_AGENT_DEPTH';
export const ELANOUS_AGENT_DEPTH_CAP_ENV = 'ELANOUS_AGENT_DEPTH_CAP';
const DEFAULT_DEPTH_CAP = 3;

export interface ElanousAsChildAdapterOpts {
  /** Path or name of the elanous binary. Default `'elanous'`. */
  readonly elanousBinary?: string;
  /** Extra args passed after `--acp-server`. Rarely needed —
   *  sub-elanous inherits the parent's cwd/env already. */
  readonly extraArgs?: readonly string[];
  /** Adapter id · default `'elanous-as-child'`. */
  readonly id?: string;
  /** Depth cap override. `ELANOUS_AGENT_DEPTH_CAP` env still wins
   *  when set; this is only the hard-coded default. */
  readonly depthCap?: number;
  /** Test seam — inject a synthetic process spawner so unit tests
   *  don't fork actual elanous processes. Receives the full argv and
...
const BRANDS = new Set(['elanous', 'elanous-child']);
const MODES = new Set(['auto', 'acp', 'hybrid']);

export function createElanousAsChildAdapter(
  opts: ElanousAsChildAdapterOpts = {},
): AgentAdapter {
  const id = opts.id ?? 'elanous-as-child';
  const binary = opts.elanousBinary ?? 'elanous';
  ...
  const launch = async (spec: AgentLaunchSpec): Promise<EmbodiedAgentSession> => {
    ...
    if (currentDepth + 1 > cap) {
      throw new Error(
        `${id} · recursion depth cap reached (current=${currentDepth} cap=${cap}) · refusing to spawn sub-elanous`,
      );
    }
    ...
    debug.log(
      'agent.elanousAsChild.launch',
      `bin=${binary} depth=${currentDepth + 1}/${cap} pid=${child.pid ?? '(unknown)'}`,
    );
    return wrapElanousChildSession(id, spec, child);
  };
...
function readDepth(env: Readonly<Record<string, string>> | undefined): number {
  const raw = env?.[ELANOUS_AGENT_DEPTH_ENV] ?? process.env[ELANOUS_AGENT_DEPTH_ENV];
  if (!raw) return 0;
...
function readDepthCap(
  ...
  const raw = env?.[ELANOUS_AGENT_DEPTH_CAP_ENV] ?? process.env[ELANOUS_AGENT_DEPTH_CAP_ENV];
...
function buildChildEnv(
  ...
  // Inherit process env first so sub-elanous sees PATH, HOME, etc.
  ...
  env[ELANOUS_AGENT_DEPTH_ENV] = String(nextDepth);
  env[ELANOUS_AGENT_DEPTH_CAP_ENV] = String(cap);
  return env;
}
...
function wrapElanousChildSession(
  adapterId: string,
  ...
  const send = async (input: string): Promise<void> => {
    ...
    if (!child.stdin || child.stdin.destroyed) {
      throw new Error('sub-elanous stdin unavailable · cannot send input');
    }
  ...
  const snapshot = async (): Promise<string> => {
    // Sub-elanous in ACP-server mode doesn't render a terminal UI · the
...
export function registerDefaultElanousAsChildAdapter(
  registry: import('../adapter-registry.js').AdapterRegistry,
  opts?: ElanousAsChildAdapterOpts,
): () => void {
  return registry.register(createElanousAsChildAdapter(opts));
}

/** Test-only · reset the module-level session sequence counter. */
export function _resetElanousAsChildSessionSeqForTesting(): void {
  sessionSeq = 0;
}
