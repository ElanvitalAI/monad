// ── `elanous self <sub>` 의 로그 sink surface 결정 (2026-07-30) ──────────────────
//
// ⛔ **왜 필요한가** — 독립 프로세스는 데몬의 StoreSink 를 상속하지 않는다. 등록 안 하면 그
// 프로세스의 `debug.log(...)` 가 **logs.db 에 안 닿아** `elanous logs` 로 조회 불가다(= 관측 안 한 것).
...
// 파이프라인을 `elanous dev` 로 타면 관측되고 `self implement` 로 타면 안 되는 **입구별 비대칭**이었다.
...
/** `elanous dev` 등록과 자식 provider 오류 조회가 함께 쓰는 sink surface. */
