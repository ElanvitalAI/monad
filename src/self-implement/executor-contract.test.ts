...
describe('executor-contract — G9 P3a 통일 계약 SSOT', () => {
  ...
  test('EXECUTOR_BACKENDS = monad-chat + agent CLI 5종(회귀 스냅샷)', () => {
    expect([...EXECUTOR_BACKENDS]).toEqual(['monad-chat', 'codex', 'claude', 'gemini', 'grok', 'aside']);
  });
  ...
  test('buildExecutorPtyRef — surfaceId 불변식 보장 + 선택 필드 부재 시 생략', () => {
    const full = buildExecutorPtyRef({ ptyId: 'goal_ff00', backend: 'monad-chat', runId: 'run-1', spaceId: 'sp-1', sessionId: 's-1', instance: 'inst-a' });
    expect(full).toEqual({ ptyId: 'goal_ff00', surfaceId: 'exec:goal_ff00', backend: 'monad-chat', runId: 'run-1', spaceId: 'sp-1', sessionId: 's-1', instance: 'inst-a' });
