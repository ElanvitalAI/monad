// 헤드리스 monad 드라이버 (2026-07-19) — "내부 drive-tui".
//
// 대표 지시: 외부 scripts/drive-tui.ts 가 하듯 monad TUI 를 PTY 로 띄워 구동하되, monad 가
// **스스로**(TUI 안에서 SelfImplement 툴 → 이 드라이버) 자식 monad 를 worktree cwd 에 띄워
// feature 를 구현시킨다. dispatchDriveCodingAgentHeadless 는 codex/claude 바이너리만 스폰하므로
// monad 자기-스폰용 전용 드라이버가 필요.
//
// 흐름: startPty(bun <repo>/bin/monad.mjs, cwd=worktree, goal-loop 아밍 config) → 부팅 대기
...
export function watchHarnessBoundaryRequests(
  ...
  if (!path) return () => {};
  const responsePath = options.responsePath ?? process.env.MONAD_HARNESS_BOUNDARY_RESPONSES;
...
export interface HeadlessMonadResult {
...
export interface HeadlessMonadOptions {
  /** main repo 절대경로(bin/monad.mjs·node_modules 해석용). */
  repoRoot: string;
  /** monad 를 띄울 작업디렉토리(worktree). codex 편집이 여기 일어남. */
  cwd: string;
  /** 구동할 feature 프롬프트. */
  prompt: string;
   ...
   *  `monad self run <runId>` 가 그 self-implement 전체를 조인한다. */
  runId?: string;
  stateDir?: string;
...
/**
 * worktree cwd 에서 헤드리스 monad 를 띄워 feature 를 구동하고 완료까지 캡처한다.
 * SelfImplement 오케스트레이터의 ③구현 seam 이 이걸 호출한다.
 *
 * ⛔⭐⭐⭐ 이 파일에는 스포너가 **둘**이고 자식의 «모양»이 다르다 — 섞으면 안 된다(2026-08-07 · `[S]` 51차).
 *   ① 이 함수 `driveHeadlessMonad`  — `monadTuiSpawnOptions` → `bun bin/monad.mjs` (서브커맨드 없음 = **TUI 모양**)
 *      아래 `h.write(opts.prompt)` ⊕ `h.write('\r')` 로 **자식에게 타이핑해 넣는다** ⇒ 입력 소비자가 **있다**.
 *      ⚠️ 그런데 이 함수의 옵션(`HeadlessMonadOptions`)엔 `canReceiveInput` 도 `autoAssist` 도 **없다**(감독 미배선).
 *   ② `runHeadlessGoalLoopPty`(아래) — `bun bin/monad.mjs chat --tools --goal-loop --new <prompt>`
 ...
 */
export async function driveHeadlessMonad(opts: HeadlessMonadOptions): Promise<HeadlessMonadResult> {
  ...
  //   env 를 그대로 읽는다). 빈 runId 는 `monad self run <runId>` 사후 join 을 **불가능**하게 만든다(관측 계약은
  ...
  try {
    ...
    phase = 'spawn';
    const h = spawn(monadTuiSpawnOptions({
      repoRoot: opts.repoRoot,
      cwd: opts.cwd,
    ...
    for (let i = 0; i < maxI; i += 1) {
      ...
      // ★ 화면 릴레이 프레임(2026-07-21) — full snapshot 을 공간 화면 버퍼에 써 프로세스 경계 넘어 `monad self
      //   screen --space <id>` 로 라이브 관측(X11 forwarding식·프레임버퍼). fail-soft 내장.
      //   (P3 manifest frame 수렴은 라이브 executor runHeadlessGoalLoopPty 에만 배선 — 이 driveHeadlessMonad
...
// startPty 로 띄운다 — driveHeadlessMonad(=`legacy` TUI·부팅+프롬프트박스 write+화면스크레이프)와
...
export interface HeadlessGoalLoopPtyOptions {
  /** main repo 절대경로(bin/monad.mjs·node_modules 해석). */
  binRoot: string;
  /** worktree(구현 대상 cwd). */
  cwd: string;
   ...
   *  스폰(resolveEscalateTarget → MONAD_ESCALATE_* env → 자식 config override). base(terra) 소진 시 sol,
  ...
  /** activityGraceSec의 구성 출처. caller는 `monad dev --activity-grace`가 전달한 값이다. */
   ...
   *   그냥 넓히면 monad 가 **없는 능력을 선언**하고, 그것이 `#5665` 가 막으려던 것이다 —
   *   *"닿지 않았는데 성공이라 보고한다."*
...
/**
 * ⭐ poll 루프의 종료 갈래(안정 식별자 · `monad logs --grep exitReason` 로 집계 가능).
 *
...
export async function runHeadlessGoalLoopPty(opts: HeadlessGoalLoopPtyOptions): Promise<HeadlessGoalLoopPtyResult> {
  ...
  const implementRole = !childLlm && !process.env.MONAD_LLM_MODEL?.trim()
    ? (() => { try { return resolveRoleLlm('implement'); } catch { return undefined; } })()
    : undefined;
  // ⭐ B9 — 승급 판정에 쓸 «유효 provider»: 명시 자식 → 역할 → 런 env → config(auto 는 실제 결정으로).
  const effectiveChildProvider = childLlm?.provider ?? implementRole?.provider ?? process.env.MONAD_LLM_PROVIDER?.trim()
  ...
  const rawArgs = [`${opts.binRoot}/bin/monad.mjs`, 'dev', '--implement', ...iso, opts.featurePrompt];
  ...
  const boundaryRequestsPath = boundaryRequestsEnv.MONAD_HARNESS_BOUNDARY_REQUESTS ?? '';
  const boundaryResponsesPath = boundaryResponsesEnv.MONAD_HARNESS_BOUNDARY_RESPONSES ?? '';
  ...
  try {
    ...
    //   ④자식의 `MONAD_PTY_ID`(=lifecycle 발행) 가 **전부 조용히 사라진다.**
    ...
    workdir: opts.cwd,
    env: {
      ...(opts.stateDir ? { MONAD_STATE_DIR: opts.stateDir } : {}),
...
// ⛔⭐ 뿌리와 «한 벌»로 간다(`OBS-T121`) — 안 주면 자식이 「파생」을 「명시 격리」로 읽는다.
            ...(opts.stateDir && opts.stateDirSource ? { MONAD_STATE_DIR_SOURCE: opts.stateDirSource } : {}),
      MONAD_PARENT_SELF_DEV_RUNS_DIR: selfDevRunsDir(),
      ...childRunContextEnv('self-build'),
      // ⭐ 구현 자식은 코딩 도구 프로필로 — 도메인·운영 도구 스키마를 매 턴 싣지 않는다(BACKLOG L1 · tool-profile.ts).
      // 기본 coding(다이어트) · 부모가 `MONAD_CHILD_TOOL_PROFILE=full` 또는 묶음(`finance,ops`)을 주면 «더한다»(tool-profile.ts).
      MONAD_TOOL_PROFILE: childToolProfile(process.env, opts.featurePrompt),
      // ⭐ 로컬 모델 자식은 시스템 프롬프트도 «lean» 예산(AGENTS.md 앵커 8K · 트리 2K) — BACKLOG L2 · universal-preamble.ts.
      ...(((childLlm?.provider ?? process.env.MONAD_LLM_PROVIDER)?.trim() === 'local') ? { MONAD_PROMPT_BUDGET: 'lean' } : {}),
      ...childNestEnv(),
      ...
      ...(opts.documentReferences?.length ? { MONAD_DOCUMENT_REFERENCES: JSON.stringify(opts.documentReferences) } : {}),
      // ⭐ BACKLOG B14 (대표 2026-09-25) — 명시 자식 LLM 도 런 단위 모델(`MONAD_LLM_MODEL`)도 없으면 역할 `implement`(better)로 고른다.
      ...
      ...((): Record<string, string> => {
        ...
        if (!target) return {};
        return {
          MONAD_ESCALATE_MODEL: target.model, MONAD_ESCALATE_PROVIDER: target.provider,
          ...(target.effort ? { MONAD_ESCALATE_EFFORT: target.effort } : {}),
  ...
  //   ⇒ 제1원칙: 조회로 확인 가능해야 한다. `monad logs --category self-implement --grep autostop.config`
  ...
  //   스탬프가 이 runId↔ptyId 를 영속화(closed_at tombstone) → `monad self run <runId>` 사후 join(K5).
  ...
  try {
    ...
    //   실측: `bun bin/monad.mjs self typecheck` = **147초 무출력**(변경 5파일) ⇒ 구조적으로 90 초과.
    ...
    pollLoop: for (let i = 0; i < hardI; i += 1) {
      ...
      //   (frozen·어느 op에 블록) vs i 증가(idle·타임아웃 로직 문제)** 를 결정적으로 판정. 뷰=`monad self screen --hb`.
      ...
      phase = 'poll';
      // ★ 화면 릴레이 프레임(X11 forwarding식) — `monad self screen --space <id> -f` 로 라이브(hang 순간 화면도). fail-soft.
