...
export interface MonadTuiSpawnOptions {
  ...
  /** ⭐ 이 자식이 살 **PTY 의 id**(hold 경로는 미리 발급한다). 자식에게 `MONAD_PTY_ID` 로 넘긴다.
...
export interface MonadTuiIsolation {
  readonly configDir: string;
  readonly stateDir: string;
 ...
 * isolated root — e.g. root `/tmp/r` with `MONAD_STATE_DIR=/tmp/r/state/existing`.
 ...
 */
export function establishMonadTuiIsolation(opts: { root?: string; callerStateDir?: string; sourceConfigDir?: string }): MonadTuiIsolation {
  let root: string;
  let owned = false;
  try {
    if (opts.root) {
      ...
      root = realpathSync(mkdtempSync(join(tmpdir(), 'monad-drive-')));
      owned = true;
    }
    ...
    if (callerState && (overlaps(configDir, callerState) || overlaps(stateDir, callerState))) {
      throw new Error(`isolated monad config/state overlaps caller state directory ${callerState}`);
    }
    ...
    throw new Error(`cannot establish isolated monad TUI root${opts.root ? ` at ${opts.root}` : ''}: ${error instanceof Error ? error.message : String(error)}`);
  }
}

/** The sole spawn recipe for a bare monad TUI child. */
export function monadTuiSpawnOptions(opts: MonadTuiSpawnOptions): StartOpts {
  const isolated = Boolean(opts.configDir && opts.stateDir);
  if (opts.requireIsolation && !isolated) {
    throw new Error('isolated monad TUI requires both configDir and stateDir');
  }
  const { runId } = resolveRunIdentity({ explicit: opts.runId, inherited: opts.space.runId });
  const debugLevel = process.env.MONAD_DEBUG_LEVEL;
  const controller = opts.controller ?? deriveChildController(process.env, process.pid);
  const channels = 'pty,inbox';
  debug.log('self-implement.monad-tui-spawn', 'controller', { controller, channels });
  return {
    cmd: 'bun',
    args: [
      `${opts.repoRoot}/bin/monad.mjs`,
...
// ⛔⭐ 뿌리와 «한 벌»로 간다(`OBS-T121`) — 안 주면 자식이 「파생」을 「명시 격리」로 읽는다.
      ...
      MONAD_CONTROLLER: controller,
      MONAD_CONTROL_CHANNELS: channels,
      ...observeOnlyFlagEnv(opts.observeOnly),
      ...(debugLevel === undefined ? {} : { MONAD_DEBUG_LEVEL: debugLevel }),
      ...
      //    (`headless-monad-driver.ts`). 그래서 `--hold` 로 띄운 TUI 는 `MONAD_PTY_ID` 가 없어
      ...
      // ⭐⭐ 경계에서 «묻는 길» — 종전엔 self-implement 경로에만 실렸다(`seams.ts` · `headless-monad-driver.ts`).
      //    그래서 `dev --monad` 자식은 경계에서 거부돼도 «요청을 남길 자리»가 없었다.
