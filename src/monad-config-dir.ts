// Centralized resolver for the monad config / daemon root directory.
//
// **Public surface (single source of truth)**:
//   - `--config-dir <dir>` CLI flag (extracted in
//     src/cli/config-dir-flag.ts) → routes through `setMonadConfigDir`
//   - `setMonadConfigDir(dir)` programmatic setter for tests + library
...
//   - `MONAD_DAEMON_DIR` env var read AND env mirror. Children inherit
//     the override via `--config-dir <dir>` re-appended in
//     `bg-launch.ts`, not via process.env.
//
// Resolution order:
//   1. Programmatic override set via `setMonadConfigDir(dir)` (incl.
//      from the `--config-dir` CLI flag).
//   2. `~/.monad` — default daily-driver root.
...
/** Resolve the current monad config / daemon root directory. Caller
 *  decides what subpath to append (`workflows/`, `config.json`,
 *  `acp-token`, ...). */
export function getMonadConfigDir(): string {
  if (override !== undefined) return override;
  // ★ §4d (P3 · 2026-07-26) — **config-dir 은 state-dir 을 따라간다.**
  //
  //   `MONAD_STATE_DIR` 은 env 라 전 자손에 자동 전파되는데 config-dir 은 argv 재부착 **7곳**
  ...
  //
  //   ⚠️ prod 루트(`~/.monad`)와 같으면 종전과 동일(무변경). 격리 루트일 때만 따라간다.
  //   설계 = 내부 문서 `DESIGN-instance-leader-and-default-test-2026-07-26` §4d.
  //   ★ **effectiveInstanceRoot 를 소비**한다 — state 축(monadStateRoot)과 **같은 함수**를 거쳐
...
export function setMonadConfigDir(dir: string): void {
  const trimmed = dir.trim();
  if (trimmed.length === 0) {
    throw new Error('setMonadConfigDir: empty directory');
  }
  override = trimmed;
}

/** Clear the override (test cleanup). Idempotent. */
export function resetMonadConfigDir(): void {
  override = undefined;
}

/** The explicit override, or undefined when running on the `~/.monad`
 *  default. Lets consumers (nexus/paths.ts) rank an EXPLICIT
 *  `--config-dir` above the legacy `MONAD_NEXUS_DIR` env fallback
 *  without changing default-path behavior. */
export function getMonadConfigDirOverride(): string | undefined {
  return override;
}
