...
//
// 세션 패브릭 = monad 가 자기 대화를 서피스 경계 넘어 인지하는 자기인지 표면. 그래서 세션
...
//   ① 로그(항상)     — debug.log('session.<subsystem>', event, payload) → logs.db → `monad logs`
//   ② 자기인지(중대)  — injectSelfMemory(importance>=THRESHOLD) → self-memory ambient 회상
//   ③ 운영전이(상태성) — recordOpsEventSafe(stateful·entityType='session') → `monad ops` timeline
//
...
export function recordSessionObservation(
  ...
  const ptyId = process.env.MONAD_PTY_ID?.trim();
  ...
  // ① 로그(항상) — `monad logs --category session.*` 로 조회 가능해짐.
  try {
  ...
  // ③ 운영전이(상태성) — ops_events(monad ops timeline).
  if (ev.stateful) {
    try {
