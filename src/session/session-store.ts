...
// (~/.elanous/sessions · index.json + <id>.jsonl)를 단일 진실원으로 감싸는 클래스.
// 함수형 src/session API 를 OO 로 래핑 + 명시 id write-through(appendById)를 추가해
// PWA 챗(ACP elanous-session-N)·텔레그램·CLI 가 모두 같은 저장소를 공유하게 한다(R3).
...
export class SessionStore {
   ...
   *  모듈 로드 이후 설정된 ELANOUS_STATE_DIR/ELANOUS_SESSION_ROOT를
...
/** 프로세스 공용 기본 매니저(~/.elanous/sessions). */
