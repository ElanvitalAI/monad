...
#
# ⛔ 이것은 «진단용»이다. 운영 경로는 monad 의 MCP 클라이언트다
#    (`monad mcp diagnose|call`). 이 스크립트는 그 클라이언트가 붙기 «전»이나
#    붙은 뒤 사람이 한 번 눌러 볼 때만 쓴다.
# ⛔ 토큰은 auth.json 에서 읽는다 — 인자로 받지 않는다(셸 이력에 남지 않게).
set -u
ROOT="${MONAD_INSTANCE_ROOT:-$(cd "$(dirname "$0")/.." && pwd)/.monad-test}"
AUTH="$ROOT/auth.json"
ISSUER="https://clerk.higgsfield.ai"
[ -f "$AUTH" ] || { echo "⛔ auth.json 없음: $AUTH — 'monad mcp login higgsfield-bridge' 를 먼저" >&2; exit 2; }
TOK=$(jq -r --arg i "$ISSUER" '.providers[$i].tokens.accessToken // empty' "$AUTH")
[ -n "$TOK" ] || { echo "⛔ $ISSUER 자격증명 없음 — 'monad mcp login higgsfield-bridge'" >&2; exit 2; }
