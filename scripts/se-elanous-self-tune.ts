#!/usr/bin/env bun
// ── Self-Evolution · elanous-self 튜닝 하니스 (2026-07-11) ────────────────────
// elanous-self(se-elanous-self-impl.ts) 단독을 격리 worktree 에서 반복 실행하며
// model × effort × maxTurns × adaptive 조합을 실측(파일/LOC/게이트/시간). sweet spot 탐색용.
// se-backend-bench.ts 의 elanous-self 전용·다축·빠른 iteration 판.
//
// 사용:
//   bun scripts/se-elanous-self-tune.ts --turns 24 --effort high --model gpt-5.6-sol [--adaptive] [--keep] [--task hello|tox]
//   bun scripts/se-elanous-self-tune.ts --matrix     # 사전정의 매트릭스 순차 실행
...
const TASK_HELLO = [
  'Create a file scripts/elanous-self-smoke.ts that exports a function add(a:number,b:number):number returning a+b,',
  'and a matching test scripts/elanous-self-smoke.test.ts using bun:test that asserts add(2,3)===5.',
...
async function runOne(c: Combo): Promise<Row> {
  ...
  const gateArgs = c.gateArgs ?? (c.task === 'hello' ? ['scripts/elanous-self-smoke.test.ts'] : ['src/task-orchestrator/']);
  ...
  try {
    ...
    if (c.model) env.ELANOUS_SELF_MODEL = c.model;
    if (c.effort) env.ELANOUS_SELF_EFFORT = c.effort;
    if (c.turns) env.ELANOUS_SELF_MAX_TURNS = c.turns;
    if (c.adaptive) { env.ELANOUS_SELF_ADAPTIVE = '1'; env.ELANOUS_SELF_VERIFY_CMD = `bun test ${gateArgs.join(' ')}`; }
    if (c.system) env.ELANOUS_SELF_SYSTEM = c.system;
    spawnSync('bun', [join(repoRoot, 'scripts/se-elanous-self-impl.ts'), task],
