#!/bin/zsh
# [pilot · 2026-07-06] Conatus 스크리너 백필 (monad-owned, 일별 누적).
#
# 배경: 캡스톤/KORU 백테스트가 읽는 screener.db(prices/screen/investor)는
# `~/.monad/conatus/`로 물리 이전됨(MIGRATION-capstone-leverage-2026-07-06 §7).
# 이관 시점 스냅샷은 17 거래일(2026-05-20~07-02)뿐이라 forward 윈도가 비어
# 있었다. 이 래퍼가 그 공백을 매일 채운다.
#
# 동작: Conatus 스킬사이드 report.py(파이썬 재사용, 대표 결정)를
# CONATUS_DATA_DIR=~/.monad/conatus 로 호출 → EODHD eod-bulk-last-day 로
# KRX 전종목 최신 EOD 를 받아 monad screener.db 에 최신 1거래일 append
...
#
# ⚠️ 백테스트 로직/파이썬은 스킬사이드(git 밖) 재사용. 이 스크립트가 monad
...
export CONATUS_DATA_DIR="$HOME/.monad/conatus"
