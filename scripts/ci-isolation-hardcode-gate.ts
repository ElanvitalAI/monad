 ...
 * 아무리 잘 만들어도 강제가 없으면 다음 스토어 작성자가 또 `join(homedir(), '.monad', …)`
 * 를 하드코딩한다 → 5번째 누출. 이 게이트가 그 "강제(③)" 조각이다.
 *
 * 정책(ci-typecheck-changed 와 동형 "부채 관용·신규 차단"): main 은 이미 다수의
 * homedir+.monad 하드코딩을 가진다(정당한 resolver 정의 + 미수복 위반 D/E/F 대상).
...
/** 드리프트 신호: 같은 줄에 `homedir()` 와 `.monad` 가 함께 = resolver 우회 후보.
 *  (env 재이중화 MONAD_HOME 등은 Phase F 스코프 — 여기선 homedir+.monad 만.) */
/** `.monad` 가 «경로 조각»으로 쓰였나 — 앞이 따옴표나 `/` 일 때만(`com.monad.nexus.plist` 같은 이름 속 `.monad` 는 아니다 · 2026-09-24 오탐). */
const MONAD_PATH_SEGMENT = /['"`/]\.monad(?:-test)?(?=['"`/]|$)/;

export function isHardcodeLine(line: string): boolean {
  if (isCommentOnlyLine(line)) return false;
  return line.includes('homedir()') && MONAD_PATH_SEGMENT.test(line);
}
 ...
 *
 *  ⚠️ **줄 «전체»가 주석일 때만** 건너뛴다. `const p = join(homedir(), '.monad'); // 메모` 처럼
...
/** `homedir()` 값을 받은 이름들(파일 단위). 🩸 2026-09-24(🅢 T3): 탐지가 «같은 줄에 `homedir()` ⊕ `.monad`» 뿐이라
 *  `const home = deps.home ?? homedir(); join(home, '.monad', …)` 처럼 «두 줄로 나눈» 하드코딩을 못 봤다
...
export function scanHardcodeCandidates(source: string): HardcodeCandidate[] {
  ...
  return source.split('\n').flatMap((line, index) => (
    isHardcodeLine(line) || (aliasPattern !== null && !isCommentOnlyLine(line) && MONAD_PATH_SEGMENT.test(line) && aliasPattern.test(line))
...
function writeBaseline(entries: Map<string, HardcodeScanEntry>): void {
  ...
  const header = [
    '# 격리 하드코딩 baseline (homedir()+.monad 라인 수 · 파일별) — Phase C ratchet 게이트.',
...
export function runIsolationHardcodeGate(io: IsolationGateIo = {}): number {
  ...
  if (violations.length > 0) {
    error('[isolation-gate] FAIL — 신규 homedir+.monad 하드코딩 감지.');
    error('  격리 누출 근본(PLAN §0): 스토어 경로는 resolver(monadStateRoot/memoryDbPath/');
    error('  getMonadConfigDir/instanceRoot)를 거쳐야 test↔prod 격리가 성립합니다.');
