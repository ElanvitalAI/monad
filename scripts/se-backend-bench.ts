...
//   ① elanous-self      : bun se-elanous-self-impl.ts (getAutopilotToolRegistry·gpt-5.6-sol)
//   ② codex-app-server: dispatchDelegateAgent (codex ACP 위임)
//   ③ claude          : dispatchDelegateAgent (claude code ACP 위임)
// 사용: bun scripts/se-backend-bench.ts [--only elanous-self|codex|claude]
...
async function runElanousSelf(plan: IsolatedPlan): Promise<void> {
  // 격리 서브프로세스(cwd=worktree)로 elanous-self 러너 스폰. 도구가 worktree 편집.
  // 튜닝 확정 config(2026-07-11·[[REPORT-elanous-self-tuning-2026-07-11]]): effort=medium·
  // maxTurns=40·optimized 프롬프트(+cwd)·검증-주도 adaptive 폐루프(verify=게이트 스코프 일치).
  // 이 config 로 elanous-self 가 원 리포트의 "예산 소진 실패"를 넘어 실 구현을 완결한다.
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    ELANOUS_SELF_MODEL: 'gpt-5.6-terra', // 튜닝 매트릭스 최적 codex(안정·최속). config 기본 sol 은 코딩 비효율.
    ELANOUS_SELF_EFFORT: 'medium',
    ELANOUS_SELF_MAX_TURNS: '40',
    ELANOUS_SELF_SYSTEM: 'optimized',
    ELANOUS_SELF_ADAPTIVE: '1',
    ELANOUS_SELF_VERIFY_CMD: 'bun test src/task-orchestrator/',
  };
...
const BACKENDS: Array<{ name: string; key: string; run: (p: IsolatedPlan) => Promise<void> }> = [
  { name: 'elanous-self', key: 'elanous-self', run: (p: IsolatedPlan) => runElanousSelf(p) },
