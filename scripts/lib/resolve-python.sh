...
#   ① $ELANOUS_PYTHON  ② elanous venv  ③ pyenv 의 <저장소>/.python-version 판  ④ PATH 의 python3
# 사용: PY="${PY:-$(sh "$(dirname "$0")/lib/resolve-python.sh")}"   · 못 찾으면 빈 줄 ⊕ rc 1
# ⛔ bun 에 기대지 않는다 — 크론의 PATH(/usr/bin:/bin)에서도 돈다.
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
if [ -n "${ELANOUS_PYTHON:-}" ] && [ -x "$ELANOUS_PYTHON" ]; then echo "$ELANOUS_PYTHON"; exit 0; fi
VENV="${XDG_DATA_HOME:-$HOME/.local/share}/elanous/python/venv/bin/python"
