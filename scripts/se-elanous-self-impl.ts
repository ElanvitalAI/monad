#!/usr/bin/env bun
// ── Self-Evolution · elanous-self 구현 러너 (백엔드 벤치용·2026-07-11) ────────
// elanous 자체 에이전트 루프(ElanousBuiltinTurnRunner + getAutopilotToolRegistry 코딩도구)로
// 격리 cwd 에서 코드를 자율 구현. 도구는 ELANOUS_TOOL_CWD/process.cwd 로 cwd 바인딩 →
// 이 스크립트를 worktree cwd(또는 ELANOUS_TOOL_CWD)로 스폰하면 그 worktree 를 편집한다.
// codex/claude ACP 위임(dispatchDelegateAgent)의 elanous-self 대응. 사용: bun se-elanous-self-impl.ts <task>
...
import { resolveSystemPrompt } from './se-elanous-self-prompts.js';
...
//   파일 트레일(buildLog)에만 남고 logs.db 에 안 닿아 `elanous logs --category mission.se.walker` 로 조회 불가
//   (= 관측 안 한 것). self-implement CLI(index.ts) 와 동일 패턴. ELANOUS_STATE_DIR 상속 → 격리 logs.db 정합.
try { await registerStandaloneLogSink('harness:se-build'); } catch { /* fail-open */ }

// ★ 근본 개선(대표 지시) — 실행 cwd 를 도구 체인에 명시 전달. elanous-self 의 code-edit 도구는
// getSessionCwd() 로 상대경로를 anchor 하고, self-edit 가드가 elanous 소스 편집을 차단한다.
// 격리 worktree(disposable·disarmed·SE arming 게이트 하)에서 elanous-self 가 자율 편집하려면:
...
//   ELANOUS_SELF_MAX_TURNS : per-prompt tool-loop 예산(명시 시 codex 패밀리 기본 8 override).
//   ELANOUS_SELF_MODEL     : modelOverride (gpt-5.6-sol / gpt-5.6-terra / anthropic/claude-opus-4-6 등).
//   ELANOUS_SELF_EFFORT    : codex reasoning effort (minimal|low|medium|high|xhigh|max) — config in-place override.
//   ELANOUS_SELF_ADAPTIVE  : '1' 이면 max_turns 도달 + 진행중일 때 예산 확장하며 재프롬프트(adaptive escalation).
const envMaxTurns = process.env.ELANOUS_SELF_MAX_TURNS ? Number(process.env.ELANOUS_SELF_MAX_TURNS) : undefined;
const envModel = process.env.ELANOUS_SELF_MODEL || undefined;
const envEffort = process.env.ELANOUS_SELF_EFFORT as 'minimal'|'low'|'medium'|'high'|'xhigh'|'max'|undefined;
const envAdaptive = process.env.ELANOUS_SELF_ADAPTIVE === '1';
const envSystem = process.env.ELANOUS_SELF_SYSTEM || undefined; // thin|optimized|action-first
// cwd 를 프롬프트에 명시 — codex 계열이 존재하지 않는 컨테이너 루트(/workspace 등)를 추측해
// "file not found" 로 tool call 을 낭비하는 것을 예방(elanous-self dogfood 2026-07-11).
...
try {
  ...
  if (envModel) {
    ...
    if (wantProvider) {
      ...
      console.error(`[elanous-self] provider 전환 → ${wantProvider} (apiKey ${keySource})`);
    }
  }
} catch (e) { console.error('[elanous-self] 격리 컨텍스트 설정 실패:', e instanceof Error ? e.message : e); }

const task = process.argv.slice(2).join(' ') || 'Create hello.txt containing ELANOUS_SELF_OK then stop.';
const signal = new AbortController().signal;
const { tools, dispatchTool } = getAutopilotToolRegistry({ surface: 'tui', sessionId: 'elanous-self-bench', signal });

// ★ 관측(제1원칙·2026-07-22) — 어느 루프(goal-loop vs custom)·모델·예산으로 구동하는지 logs.db 에.
//   `elanous logs --category mission.se.walker` 로 "walker 가 뭘로 어떻게 돌았나" 조회(종전 stderr 사각 해소).
const goalLoopMode = process.env.ELANOUS_SELF_GOALLOOP === '1';
debug.log('mission.se.walker', 'start', { loop: goalLoopMode ? 'goal-loop' : 'custom-escalation', model: envModel ?? '(default)', maxTurns: envMaxTurns ?? null, adaptive: envAdaptive, verify: process.env.ELANOUS_SELF_VERIFY_CMD ? true : false });

// ★ 이식 #2 (canonical goal-loop → se build · 2026-07-22 대표 지시 · RESEARCH-porting-goalloop-harness) —
//   opt-in(ELANOUS_SELF_GOALLOOP=1): 자체 escalation 루프(아래) 대신 canonical runGoalLoop 로 구동한다.
//   흡수 능력: 증거게이트 완주(update_goal+evidence — "테스트 안 돌리고 done" 위증 차단)·구조화
//   `goal.loop` 관측(elanous logs 조회)·anti-spin(동일 tool 시그니처 정체감지)·context 압력 관리·구조화
//   stopReason. verify 는 모델이 도구로 실행해 evidence 로 인용(nocturnal-runner gate 가 최종 검증).
//   기본(미설정)=ElanousBuiltinTurnRunner 커스텀 루프(무회귀). getAutopilotToolRegistry 반환이 이미
//   {LLMToolSpec[], CoreTurnDispatchTool} 라 래핑 없이 CoreTurnContext 에 직결.
if (process.env.ELANOUS_SELF_GOALLOOP === '1') {
  const verifyCmd = process.env.ELANOUS_SELF_VERIFY_CMD;
  ...
  const coreCtx: CoreTurnContext = {
    sessionId: 'elanous-self-goalloop',
    ...
    //   ladder 값 캡 제거. env(ELANOUS_SELF_MAX_TOOL_TURNS) 로만 조정 가능.
    maxToolTurns: Number(process.env.ELANOUS_SELF_MAX_TOOL_TURNS) || 100000,
  };
  ...
  //   페이즈는 ~3턴에 완주하므로 12면 충분·못 끝내면 mis-scope 신호). env(ELANOUS_SELF_MAX_ITERATIONS) 로 조정.
  const maxIter = Number(process.env.ELANOUS_SELF_MAX_ITERATIONS) || 12;
  console.error(`[elanous-self] ★ goal-loop 엔진(canonical·증거게이트·fail-fast→coordinator) maxIter=${maxIter} model=${envModel ?? '(default)'}`);
  const gl = await runGoalLoop(coreCtx, { maxIterations: maxIter });
  console.log(`\n[elanous-self] goal-loop stopReason=${gl.stopReason} · iterations=${gl.iterations} · toolCalls=${glToolCalls}`);
...
console.error(`[elanous-self] config: model=${envModel ?? '(default)'} effort=${envEffort ?? '(config)'} maxTurns=${envMaxTurns ?? '(family default)'} adaptive=${envAdaptive} system=${envSystem ?? 'thin'}`);

const runner = new ElanousBuiltinTurnRunner({
  sessionId: 'elanous-self-bench',
  tools,
  dispatchTool,
  systemPrompt,
  ...(envModel ? { modelOverride: envModel } : {}),
...
const verifyCmd = process.env.ELANOUS_SELF_VERIFY_CMD || undefined;
const MAX_ESCALATIONS = Number(process.env.ELANOUS_SELF_MAX_ESCALATIONS ?? 4);
...
if (envAdaptive) {
  ...
  while (escalations < MAX_ESCALATIONS) {
    if (verifyCmd) {
      ...
      console.error(`\n[elanous-self] verify(${verifyCmd}) → ${v.passed ? 'PASS ✅' : 'FAIL ❌'}`);
      ...
      if (v.passed) { console.error('[elanous-self] 검증 통과 — 완료'); break; }
      escalations++;
      console.error(`[elanous-self] ⟳ escalation ${escalations}/${MAX_ESCALATIONS} — 실패 출력 되먹임·계속`);
      ...
      escalations++;
      console.error(`\n[elanous-self] ⟳ escalation ${escalations}/${MAX_ESCALATIONS} (max_turns hit·toolCalls=${toolCalls}) — 예산 확장·계속`);
      result = await runner.prompt(
    ...
    if (toolCalls === lastToolCalls) { console.error('[elanous-self] 진행 없음(toolCalls 정체) — escalation 중단'); break; }
    lastToolCalls = toolCalls;
  }
  console.error(`[elanous-self] escalations=${escalations}`);
}

console.log(`\n[elanous-self] stopReason=${result.stopReason} · toolCalls=${toolCalls}`);
