...
const backup = resolve(import.meta.dir, 'monad-backup.sh');

async function runBackup(crontab: string, directories: string[] = []) {
  const root = await mkdtemp(join(tmpdir(), 'monad-backup-'));
  const home = join(root, 'home');
  const bin = join(root, 'bin');
  await mkdir(home, { recursive: true });
  await Promise.all(directories.map((directory) => mkdir(join(home, '.monad', directory), { recursive: true })));
...
describe('monad-backup crontab snapshot', () => {
  ...
  test('refuses to stage a crontab that carries a credential — and never prints the value', async () => {
    ...
    const { status, output } = await runBackup(`20 4 * * * AWS_ACCESS_KEY_ID=${secret} /opt/monad/run\n`);
  ...
  test('a lowercase credential name is caught too — and never printed', async () => {
    ...
    const { status, output } = await runBackup(`20 4 * * * aws_secret_access_key=${secret} /opt/monad/run\n`);
  ...
  test('an absolute path with a long first segment is NOT a credential (⛔ 첫 판이 여기서 «가려졌다»)', async () => {
    const { status, output } = await runBackup('20 4 * * * API_KEY_FILE=/abcdefghijklmnop /opt/monad/run\n');
  ...
  test('a ~ / $ shaped value is NOT a credential either', async () => {
    const { status, output } = await runBackup('20 4 * * * SECRET_FILE=~/.config/monad/abcdefghijklmnop TOKEN_REF=$MONAD_TOKEN_ABCDEFGH /opt/monad/run\n');
