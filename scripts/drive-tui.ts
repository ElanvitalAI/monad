// ── External monad TUI driver (self-dogfood harness · PoC 2026-07-17) ──
//
// 외부 에이전트(Claude Code/Codex)가 monad TUI 를 100% 실물로 구동·캡처하기
// 위한 드라이버. monad 의 `pty-shell/registry`(startPty + xterm 에뮬레이터)를
// **라이브러리로 재사용** — monad-in-monad 가 아니라 별도 driver 프로세스가
// 실제 monad TUI 를 PTY 안에서 돌리고 화면을 캡처한다. 구조 무손상.
//
// 사용:
...
async function main(): Promise<void> {
  ...
  // --cmd "<command>" — monad TUI 대신 임의 명령/TUI 구동(htop·top·vim·lazygit).
  // 이 경우 prompt 없이 boot 캡처 = 그 TUI 화면 캡처. 복잡 TUI 렌더 검증용.
  const cmdIdx = args.indexOf('--cmd');
  const cmdArg = cmdIdx >= 0 ? (args[cmdIdx + 1] ?? '') : '';
  // --test — 격리 인스턴스(<repo>/.monad-test) 로 monad TUI 구동. config·state·logs 전부
  // 격리 루트로. 운영 config·세션 무접촉(codex 병리 dogfood 안전). 선행: `bun run
  // bin/monad.mjs config sync-test` 로 격리 config 시딩(terra/high). (2026-07-19 goal-exec)
  const testMode = args.includes('--test');
  const testDirIdx = args.indexOf('--test-dir');
  const testDir = testDirIdx >= 0 ? (args[testDirIdx + 1] ?? `${process.cwd()}/.monad-test`) : `${process.cwd()}/.monad-test`;
  ...
  // --worktree — disposable git worktree 자동생성(.monad-test/worktrees·gitignore)해 그 안에서 구동.
  //   둘 중 하나면 bin/monad.mjs 는 repo 절대경로로 실행(main node_modules 해석)·cwd 만 workdir 로.
  ...
  //   멀티라인 전문의 내부 \n 이 조기 Enter(제출)로 새는 걸 막는다(monad TUI 는 200~/201~ 사이를
  ...
  // spawn 대상: --cmd 있으면 그 명령(sh -c 로 파이프/인자 지원), 없으면 monad TUI.
  // 격리 플래그는 Commander 이전에 파싱되므로(src/index.ts:2-12) monad TUI args 최상단에.
  ...
  if (explicitWorkdir) {
    ...
    workdir = `${repoRoot}/.monad-test/worktrees/drive-${Date.now()}`;
  ...
  console.error(`[drive-tui] spawning ${cmdArg || 'monad TUI'} in a PTY (${cols}x${rows})…`);
  const h: PtyHandle = startPty({
    ...
    env: { ...process.env, MONAD_DRIVE_TUI: '1', ...(testMode ? { MONAD_STATE_DIR: testDir } : {}) },
  });
  ...
  if (logsMode) {
    ...
    try {
      ...
      const logArgs = ['run', 'bin/monad.mjs', 'logs', ...(testMode ? ['--test'] : []), '--category', 'tool-loop,llm.stream,llm.router', '-n', '80'];
