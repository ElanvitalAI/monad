...
// `monad.nexus.baseUrl` localStorage key) into the
...
export function NexusClientProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // come via the storage / `monad.daemon.changed` events below.
    ...
    // `dispatchEvent(new Event('monad.daemon.changed'))` to nudge. We
    // listen to both so a settings save propagates.
    const onAny = (): void => setSnap(readSnapshot());
    window.addEventListener('storage', onAny);
    window.addEventListener('monad.daemon.changed', onAny);
    return () => {
      window.removeEventListener('storage', onAny);
      window.removeEventListener('monad.daemon.changed', onAny);
    };
  }, []);
