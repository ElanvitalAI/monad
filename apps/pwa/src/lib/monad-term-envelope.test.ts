/**
 * WT-S-1 envelope codec contract — `monad/term/*` parser. Daemon-side
 * counterpart: `src/acp/monad-extensions.ts` (두 copy 가 wire 정확히 같아야).
 *
...
describe('parseMonadTermEnvelope', () => {
  ...
  it('returns null when the first line lacks the sentinel', () => {
    expect(parseMonadTermEnvelope('plain text')).toBeNull();
    expect(parseMonadTermEnvelope('')).toBeNull();
  });

  it('returns null on unknown method', () => {
    const text = `[monad/term/unknownMethod] term-1\n{"terminalId":"term-1"}\n<<monad-term-end term-1>>`;
    expect(parseMonadTermEnvelope(text)).toBeNull();
  });

  it('returns null on malformed JSON body', () => {
    const text = `[monad/term/terminalOutput] term-1\n{not json\n<<monad-term-end term-1>>`;
    expect(parseMonadTermEnvelope(text)).toBeNull();
  });
  ...
  it('returns null when terminalInputActivity is missing peerId / timestamp / bytes', () => {
    expect(parseMonadTermEnvelope(envelope('terminalInputActivity', {
      terminalId: 'term-1',
      timestamp: 1, bytes: 1,
    }))).toBeNull();
    expect(parseMonadTermEnvelope(envelope('terminalInputActivity', {
