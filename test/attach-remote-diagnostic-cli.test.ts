...
//
// 🚨 계기(2026-08-31 · 135차 A2): `monad attach --host` 가 산출도 종료 코드도 없이 «영원히» 매달렸다.
...
describe('monad attach --host — 서버가 답하지 않을 때 «사람 문면»으로 끝난다', () => {
  ...
  test('🎯 종료 코드가 0 이 아니고, «어느 단계»인지 말하고, 스택 트레이스가 «아니다»', async () => {
    const proc = Bun.spawn(['bun', 'bin/monad.mjs', 'attach',
    ...
    expect(out).toContain('monad logs --category acp');
    // ⛔ 날 스택 트레이스로 흘리지 않는다
