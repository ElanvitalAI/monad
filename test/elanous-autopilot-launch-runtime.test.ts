...
import { dispatchElanousAutopilotLaunch } from '../src/tool-runtime/elanous-autopilot-launch-runtime.js';
...
describe('elanous_autopilot_launch goal-file preflight', () => {
  ...
  test('goal file 없이 부르면 우회를 관측에 남긴다', async () => {
    ...
    try {
      await dispatchElanousAutopilotLaunch(
        { mission: 'implement safely' },
  ...
  test('resolves a relative goal file against the launch cwd rather than process cwd', async () => {
    const launchCwd = mkdtempSync(join(tmpdir(), 'elanous-launch-context-'));
  ...
  test('warns for non-blocking canonical-section omissions and blocks reversals before spawning', async () => {
    ...
    const reversed = await dispatchElanousAutopilotLaunch(
