// MONAD_STATE_DIR — the unified isolated-state knob. When set, ALL mutable
// stores (sessions, acp-sessions, surface_events, codex-threads) relocate
// under it, so an isolated process (e.g. `monad telegram-test`) never
...
const ORIG = process.env.MONAD_STATE_DIR;
const ORIG_SESS = process.env.MONAD_SESSION_ROOT;
afterEach(() => {
  if (ORIG === undefined) delete process.env.MONAD_STATE_DIR; else process.env.MONAD_STATE_DIR = ORIG;
  if (ORIG_SESS === undefined) delete process.env.MONAD_SESSION_ROOT; else process.env.MONAD_SESSION_ROOT = ORIG_SESS;
});

const DIR = '/tmp/monad-state-test-xyz';

describe('MONAD_STATE_DIR relocates every mutable store', () => {
  test('sessions', async () => {
  ...
  test('acp session store (chat→session map)', async () => {
    ...
    // with no explicit path lands under MONAD_STATE_DIR.
    const store = new AcpSessionStore();
    expect((store as unknown as { path: string }).path).toBe(join(DIR, 'acp-sessions.json'));
  });

  test('MONAD_SESSION_ROOT still wins over MONAD_STATE_DIR for sessions', async () => {
    process.env.MONAD_STATE_DIR = DIR;
    process.env.MONAD_SESSION_ROOT = '/tmp/explicit-sessions';
    const { sessionRoot } = await import('../src/session/index.js');
    expect(sessionRoot()).toBe('/tmp/explicit-sessions');
  });

  test('unset → falls back to the homedir default (prod unaffected)', async () => {
    // ⚠️ bun test 전역 preload(bunfig.toml)가 MONAD_STATE_DIR 을 tmp 로 강제하지만,
    ...
    delete process.env.MONAD_SESSION_ROOT;
    const { surfaceEventsDbPath } = await import('../src/domains/surface-events.js');
    expect(surfaceEventsDbPath()).toContain('.monad/memory/surface_events.db');
  });
});

// Legacy-session migration must NOT run into an explicitly isolated store —
// otherwise `monad telegram-test` (MONAD_STATE_DIR) gets ~90 prod sessions
...
describe('migrateLegacySessions skips isolated stores', () => {
  ...
  test('a populated legacy store does NOT bleed into a MONAD_STATE_DIR store', () => {
    tmp = mkdtempSync(join(tmpdir(), 'migrate-skip-'));
    // Populate a legacy store (legacySessionRoot honors XDG_DATA_HOME).
    const legacy = join(tmp, 'xdg', 'monad', 'sessions');
