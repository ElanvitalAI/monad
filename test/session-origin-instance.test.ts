// 세션 출처 인스턴스 태깅 (2026-07-16) — 멀티 모나드(글로벌 데몬 + 폴더별 --test)에서
...
//   env 없이도 리졸버가 `.monad-test` 를 준다. 이름만 env 로 남겨두니 **test 스토어에 쓰면서
...
//
// 저장 위치(MONAD_SESSION_ROOT)와 인스턴스 이름(state 루트)을 독립 제어 —
...
const PROD_ROOT = join(homedir(), '.monad');
...
afterEach(async () => {
  ...
  if (ORIG_STATE === undefined) delete process.env.MONAD_STATE_DIR; else process.env.MONAD_STATE_DIR = ORIG_STATE;
...
describe('resolveInstanceName — 격리 경계 = 리졸브된 state 루트', () => {
  test('운영 루트 → prod (글로벌 데몬·크론 은 전부 공유 = prod)', async () => {
    process.env.MONAD_STATE_DIR = PROD_ROOT;
  ...
  //
  // 실측 사건(2026-07-27): 3층 스위치 ON + 비-리더 트리에서 경로는 `.monad-test` 인데 이름만
  // 'prod' 로 남아, `monad logs` 연합 뷰가 내 우주와 운영을 **둘 다 ⟨prod⟩** 로 찍었다.
    ...
    try {
      ...
      const r = spawnSync('bun', ['-e', script], {
        ...
        env: {
          ...
          MONAD_STATE_DIR: '', MONAD_CONFIG_DIR: '', MONAD_NEXUS_DIR: '', MONAD_SESSION_ROOT: '',
        },
      });
      const out = JSON.parse((r.stdout ?? '').trim().split('\n').pop() ?? '{}') as { n?: string; r?: string };
      expect(out.r?.endsWith('.monad-test')).toBe(true);   // 경로는 격리로 갔고
...
describe('createSession stamps originInstance + listSessions filters', () => {
  ...
  test('MONAD_STATE_DIR 격리 → test:<repo> 로 태깅(저장은 SESSION_ROOT)', async () => {
    process.env.MONAD_STATE_DIR = '/Users/x/source/foo/.monad-test';
...
describe('dispatchSessionQuery — instance 필터(list)', () => {
  test('list action 이 instance 로 필터', async () => {
    ...
    process.env.MONAD_STATE_DIR = '/Users/x/source/bar/.monad-test';
