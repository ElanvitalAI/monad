// ⭐ 72차 인시던트 ⑷: `MONAD_CODEX_ACCOUNT=team` 만 주면 홈을 몰라 ***조용히 default 로 떨어지는데***
...
describe('resolveCodexAccount — 정본 기록이 아는 홈', () => {
  ...
  test('env 홈이 있으면 그것이 정본보다 먼저다', () => {
    const resolved = resolveCodexAccount(
      env({ MONAD_CODEX_ACCOUNT: 'team', MONAD_CODEX_ACCOUNT_HOME: '/homes/from-env' }),
  ...
  test('정본도 모르면 종전대로 default 로 떨어진다', () => {
    const resolved = resolveCodexAccount(env({ MONAD_CODEX_ACCOUNT: 'team' }), { storedHome: () => undefined });
    expect(resolved.name).toBe(DEFAULT_CODEX_ACCOUNT);
    expect(resolved.storeKey).toBe('openai-codex');
  });

  test('심을 «안 주면» 종전 동작 그대로(하위호환)', () => {
    expect(resolveCodexAccount(env({ MONAD_CODEX_ACCOUNT: 'team' })).name).toBe(DEFAULT_CODEX_ACCOUNT);
  });

  // ⛔ 빈 문자열·공백은 「모른다」다 — 그것을 홈으로 쓰면 «없는 곳»을 가리킨다.
  test.each([[''], ['   ']])('정본이 빈 값(%p)을 주면 홈을 «모르는 것»으로 본다', (stored) => {
    expect(resolveCodexAccount(env({ MONAD_CODEX_ACCOUNT: 'team' }), { storedHome: () => stored }).name)
