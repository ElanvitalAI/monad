...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'elanous-answer-'));
...
describe('expression/config/answer-file', () => {
  test('loadAnswerFile() returns {} when the omitted default file is absent', () => {
    const previous = process.env.ELANOUS_SETUP_ANSWERS;
    const defaultPath = join(tmpDir, 'does-not-exist.json');
    try {
      process.env.ELANOUS_SETUP_ANSWERS = defaultPath;
      expect(loadAnswerFile()).toEqual({});
    } finally {
      if (previous === undefined) delete process.env.ELANOUS_SETUP_ANSWERS;
      else process.env.ELANOUS_SETUP_ANSWERS = previous;
    }
  });
  ...
  test('defaultAnswerFilePath() honours ELANOUS_SETUP_ANSWERS env', () => {
    const prev = process.env.ELANOUS_SETUP_ANSWERS;
    try {
      process.env.ELANOUS_SETUP_ANSWERS = '/tmp/alt.json';
      expect(defaultAnswerFilePath()).toBe('/tmp/alt.json');
    } finally {
      if (prev === undefined) delete process.env.ELANOUS_SETUP_ANSWERS;
      else process.env.ELANOUS_SETUP_ANSWERS = prev;
    }
  });

  test('defaultAnswerFilePath() honours XDG_CONFIG_HOME', () => {
    const prevXdg = process.env.XDG_CONFIG_HOME;
    const prevEnv = process.env.ELANOUS_SETUP_ANSWERS;
    try {
      delete process.env.ELANOUS_SETUP_ANSWERS;
      process.env.XDG_CONFIG_HOME = '/custom/cfg';
      expect(defaultAnswerFilePath()).toBe('/custom/cfg/elanous/setup-answers.json');
    } finally {
      if (prevXdg === undefined) delete process.env.XDG_CONFIG_HOME;
      else process.env.XDG_CONFIG_HOME = prevXdg;
      if (prevEnv === undefined) delete process.env.ELANOUS_SETUP_ANSWERS;
      else process.env.ELANOUS_SETUP_ANSWERS = prevEnv;
    }
  });
});
