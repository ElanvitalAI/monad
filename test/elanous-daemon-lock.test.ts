// MVP M1.2 — Elanous daemon lock + paths smoke test.
//
...
beforeEach(() => {
  tmp = mkdtempSync(joinPath(tmpdir(), 'elanous-daemon-lock-test-'));
});
...
describe('elanous-daemon lock primitives (via telegram-lock under tmp dir)', () => {
  test('acquire writes a pid file, release removes it', () => {
    const lockPath = joinPath(tmp, 'elanous.pid');
...
describe('elanous-daemon paths', () => {
  test('exposes socket / lock / log path getters', async () => {
    const mod = await import('../src/elanous-daemon.js');
    expect(typeof mod.elanousDaemonSocketPath()).toBe('string');
    expect(mod.elanousDaemonLockPath().endsWith('elanous.pid')).toBe(true);
    expect(mod.elanousDaemonLogPath().endsWith('elanous.log')).toBe(true);
  });

  test('socket path matches boot/acp-server defaultUnixSocketPath', async () => {
    const daemon = await import('../src/elanous-daemon.js');
    const boot = await import('../src/boot/acp-server.js');
    expect(daemon.elanousDaemonSocketPath()).toBe(boot.defaultUnixSocketPath());
  });
});
