...
// #5262). 근본은 `monadStateRoot()`(src/autopilot/state-paths.ts) 등 resolver 가
// MONAD_STATE_DIR 미설정 시 `~/.monad`(운영)를 반환하고, `bun test` 프로세스에
// 전역 격리 훅이 없어 격리가 **테스트 파일 개개의 자율 규율**에만 의존한 데 있다.
// 개별 스토어마다 인라인 `NODE_ENV==='test'` 리다이렉트(안전망)를 흩뿌리면 새
// 스토어가 가드를 빠뜨릴 때 또 샌다.
//
// 이 preload = **최상단 3층(계약)**: bun test 프로세스 전체에 격리 MONAD_STATE_DIR 을
// 강제해, 개별 모듈 규율과 무관하게 일괄 격리한다. 개별 안전망을 대체가 아니라
// **보완**(최종 방어선)한다 — resolver 는 첫 순위로 process.env.MONAD_STATE_DIR 을
// 존중하므로(state-paths·session·conatus·pty-manifest 동형), 여기서 세팅하면 전
// 스토어가 그 하위로 일관되게 격리된다.
//
// ⚠️ **미설정일 때만** 강제한다(`??=` 의미). 개별 테스트(monad-state-dir-isolation·
// instance identity·fleet 등)가 프로세스 시작 후 자기 MONAD_STATE_DIR 을 세팅/복원/
// 삭제하는 것을 절대 덮으면 안 된다 — 무회귀의 핵심.
//
// ⚠️ 값은 tmpdir() 하위(`monad-test-global-<pid>`) — 기존 안전망(session·conatus 의
// `tmpdir()/monad-test-*-<pid>`)과 동형. `homedir()`+`.monad` 하드코딩은 격리 게이트
...
//   `monad provider codex usage` 가 «둘 다» 401 로 죽었다.
...
if (!process.env.CODEX_HOME?.trim()) {
  const isolatedCodex = join(tmpdir(), `monad-test-codex-home-${process.pid}`);
...
// ssh fleet 은 «설정»이다(~/.monad/ssh-hosts.json) — 시험이 운영자의 실제 fleet 을 읽지 않게
// 없는 경로로 못 박는다. fleet 이 필요한 시험은 `setSshHostsForTesting(TEST_FLEET)` 로 주입한다.
process.env.MONAD_SSH_HOSTS_PATH = join(tmpdir(), `monad-test-ssh-hosts-${process.pid}`, 'absent.json');
delete process.env.MONAD_MEDIA_HOST;

const inheritedHarnessRun = Boolean(process.env.MONAD_RUN_ID?.trim());
if (inheritedHarnessRun || !process.env.MONAD_STATE_DIR?.trim()) {
  const isolated = join(tmpdir(), `monad-test-global-${process.pid}`);
