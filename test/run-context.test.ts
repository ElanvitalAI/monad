...
const orig = process.env.ELANOUS_RUN_CONTEXT;
afterEach(() => {
  if (orig === undefined) delete process.env.ELANOUS_RUN_CONTEXT;
  else process.env.ELANOUS_RUN_CONTEXT = orig;
});

describe('run-context (⑨ 서피스 적응)', () => {
  test('기본 production·비자율', () => {
    delete process.env.ELANOUS_RUN_CONTEXT;
  ...
  test('self-build/benchmark/simulator = 자율(서피스 억제 대상)', () => {
    for (const ctx of ['self-build', 'benchmark', 'simulator'] as const) {
      process.env.ELANOUS_RUN_CONTEXT = ctx;
  ...
  test('알 수 없는 값은 production 으로 방어', () => {
    process.env.ELANOUS_RUN_CONTEXT = 'garbage';
    expect(getRunContext()).toBe('production');
    expect(isAutonomousRunContext()).toBe(false);
  });

  test('childRunContextEnv 전파', () => {
    expect(childRunContextEnv('self-build')).toEqual({ ELANOUS_RUN_CONTEXT: 'self-build' });
  });
});
