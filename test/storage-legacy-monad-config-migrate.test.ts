...
//   of ~/.config/monad/config.json (Path A · 19 keys) into
//   ~/.monad/config.json (Path B · NEXUS schema).
...
import {
  migrateLegacyXdgUserConfig, __resetMonadConfigMigrateForTests,
} from '../src/storage/legacy-monad-config-migrate';
...
function legacyPath(): string {
  return join(home, '.config', 'monad', 'config.json');
}
function unifiedPath(): string {
  return join(home, '.monad', 'config.json');
}
function writeLegacy(json: unknown): void {
  mkdirSync(join(home, '.config', 'monad'), { recursive: true });
...
beforeEach(() => {
  ...
  __resetMonadConfigMigrateForTests();
});
