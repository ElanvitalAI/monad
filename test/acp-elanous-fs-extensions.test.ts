...
// the daemon-side ACP foundation that powers elanous/fs/list extension +
// elanous/fs/read · elanous/fs/stat · elanous/obsidian/info. Tests the helper
...
describe('fs-roots · backup discovery (HOME-rerouted scenario)', () => {
  ...
  // ~/.elanous has no live config.json but does have a backup carrying an
  ...
  beforeEach(() => {
    ...
    tmpHome = mkdtempSync(join(tmpdir(), 'elanous-fsroots-test-'));
    // Create a fake vault.
    vaultDir = join(tmpHome, 'MyVault');
    mkdirSync(vaultDir, { recursive: true });
    writeFileSync(join(vaultDir, 'note.md'), '# hello\n');
    // Drop a backup-shaped config under ~/.elanous (no live config.json).
    const elanousDir = join(tmpHome, '.elanous');
    mkdirSync(elanousDir, { recursive: true });
    writeFileSync(
      join(elanousDir, 'config.json.bak'),
