...
import { parseMonadUri } from '../../../src/mss/uri/parser.js';
import type { MonadUri } from '../../../src/mss/uri/brand.js';

describe('mss uri builder', () => {
  describe('newMonadUri', () => {
    test('fresh kind + ULID → Tier 2 shape', () => {
      const uri = newMonadUri('session');
    ...
    test('with parent appends segment', () => {
      const parent = newMonadUri('session');
      const child = newMonadUri('msg', parent);
      const parsed = parseMonadUri(child);
    ...
    test('unknown kind throws', () => {
      expect(() => newMonadUri('nope' as 'session')).toThrow('Unknown EntityKind');
    });
  });

  describe('joinMonadUri', () => {
    test('Tier 2 parent → Tier 2 child', () => {
      const parent = newMonadUri('session');
      const child = joinMonadUri(parent, 'block');
      expect(parseMonadUri(child)?.tier).toBe(2);
    });

    test('Tier 3 parent with fragment — fragment preserved after join', () => {
      const monadId = '01ARZ3NDEKTSV4RRFFQ69G5FAV';
      const parent = asMonadUri(`monad://local/${monadId}/session/01ARZ3NDEKTSV4RRFFQ69G5FA1#anchor`);
      const child = joinMonadUri(parent, 'msg', '01ARZ3NDEKTSV4RRFFQ69G5FA2');
      expect(child).toContain('/msg/01ARZ3NDEKTSV4RRFFQ69G5FA2');
      expect(child).toMatch(/#anchor$/);
    });

    test('joining onto Tier 1 throws', () => {
      const short = asMonadUri('ses_01HYZA');
      expect(() => joinMonadUri(short, 'msg')).toThrow(/Tier 1/);
    });

    test('explicit id respected', () => {
      const parent = newMonadUri('session');
      const child = joinMonadUri(parent, 'msg', 'fixed-id');
  ...
  describe('toTier1 / toTier2 / toTier3 round-trip', () => {
    test('Tier 2 → Tier 3 → Tier 2', () => {
      const t2 = newMonadUri('session');
      const t3 = toTier3(t2, 'local', '01ARZ3NDEKTSV4RRFFQ69G5FAV');
      const t2b = toTier2(asMonadUri(t3));
    ...
    test('toTier2 on Tier 1 throws (registry lookup required)', () => {
      expect(() => toTier2(asMonadUri('ses_01HYZA'))).toThrow(/registry/);
    });

    test('toTier3 preserves query + fragment', () => {
      const monadId = '01ARZ3NDEKTSV4RRFFQ69G5FAV';
      const t3 = asMonadUri(`monad://local/${monadId}/msg/01ARZ3NDEKTSV4RRFFQ69G5FA2?role=user#L5`);
      const again = toTier3(t3, 'local', monadId);
    ...
    test('toTier1 collapses to the leaf segment kind', () => {
      const parent = newMonadUri('session');
      const child = joinMonadUri(parent, 'msg');
  ...
  describe('asXUri — runtime validation + brand', () => {
    test('asMonadUri round-trips a valid URI', () => {
      const s = 'session/01ARZ3NDEKTSV4RRFFQ69G5FAV';
      const branded = asMonadUri(s);
    ...
    test('asMonadUri on invalid throws', () => {
      expect(() => asMonadUri('not a uri at all')).toThrow(/Invalid MonadUri/);
    });
  });

  describe('monotonicity', () => {
    test('rapid newMonadUri within same ms yields distinct ids', () => {
      const a = newMonadUri('msg');
      const b = newMonadUri('msg');
      expect(a).not.toBe(b);
      const aId = parseMonadUri(a)!.segments[0]!.id;
      const bId = parseMonadUri(b)!.segments[0]!.id;
      expect(aId).not.toBe(bId);
    });
  });

  describe('branded type compile-time guards', () => {
    test('MonadUri is assignable from runtime validator only (type-level)', () => {
      ...
      // that accepts MonadUri.
      const wantsMonadUri = (u: MonadUri): string => u as string;
      const v = newMonadUri('session');
      expect(typeof wantsMonadUri(v)).toBe('string');
    });
  });
});
