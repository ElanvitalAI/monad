...
//   ~/.config/monad/<subdir>/ → ~/.monad/<subdir>/.
...
describe('migrateLegacyXdgFile (FU2)', () => {
  function legacyFile(name: string): string {
    return join(home, '.config', 'monad', name);
  }
  function newFile(name: string): string {
    return join(home, '.monad', name);
  }
  ...
  test('migrates single file with default 0o600 mode', () => {
    require('node:fs').mkdirSync(join(home, '.config', 'monad'), { recursive: true });
  ...
  test('skips when target already exists (no clobber)', () => {
    require('node:fs').mkdirSync(join(home, '.config', 'monad'), { recursive: true });
    require('node:fs').mkdirSync(join(home, '.monad'), { recursive: true });
...
describe('migrateLegacyHomeFile (FU2 Tier 2)', () => {
  test('migrates ~/.config/monad-agent/<file> → ~/.monad/<file>', () => {
    ...
    expect(existsSync(join(home, '.monad', 'hints.json'))).toBe(true);
    expect(require('node:fs').readFileSync(join(home, '.monad', 'hints.json'), 'utf8')).toBe('"hint"');
  ...
  test('idempotent across distinct legacy roots', () => {
    ...
    writeFileSync(join(home, '.config', 'monad', 'b.json'), '"monad"');
    ...
    migrateLegacyHomeFile({
      legacyHomeRel: join('.config', 'monad', 'b.json'),
      monadRel: 'b.json',
    });

    expect(require('node:fs').readFileSync(join(home, '.monad', 'a.json'), 'utf8')).toBe('"agent"');
    expect(require('node:fs').readFileSync(join(home, '.monad', 'b.json'), 'utf8')).toBe('"monad"');
  });
});

describe('migrateLegacyHomeDir (FU2 Tier 3)', () => {
  test('migrates ~/.monad-agent/audit/ → ~/.monad/audit/', () => {
    ...
    expect(existsSync(join(home, '.monad', 'audit', 'control-2026-05-10.ndjson'))).toBe(true);
  ...
  test('skips when target dir already has files', () => {
    require('node:fs').mkdirSync(join(home, '.monad-agent', 'audit'), { recursive: true });
    require('node:fs').mkdirSync(join(home, '.monad', 'audit'), { recursive: true });
    writeFileSync(join(home, '.monad-agent', 'audit', 'old.ndjson'), 'old');
    writeFileSync(join(home, '.monad', 'audit', 'new.ndjson'), 'new');

    migrateLegacyHomeDir({ legacyHomeRel: join('.monad-agent', 'audit'), monadRel: 'audit' });

    expect(existsSync(join(home, '.monad', 'audit', 'new.ndjson'))).toBe(true);
    expect(existsSync(join(home, '.monad', 'audit', 'old.ndjson'))).toBe(false);
