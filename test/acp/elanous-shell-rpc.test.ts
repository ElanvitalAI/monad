// ── A1 (Phase 2 Bundle 3) — elanous-shell-rpc tests ──

import { describe, expect, test } from 'bun:test';
import {
  createElanousShellRpcHandlers,
  ShellRpcDeniedError,
  ShellRpcNotFoundError,
  ELANOUS_SHELL_RPC_METHODS,
} from '../../src/acp/elanous-shell-rpc';
import type {
  ShellHandle,
  ShellRegistry,
  ShellRequest,
...
describe('METHODS constants', () => {
  test('exports namespaced JSON-RPC method names', () => {
    expect(ELANOUS_SHELL_RPC_METHODS).toEqual({
...
describe('spawn', () => {
  test('vw + non-interactive → allowed by default policy', async () => {
    ...
    const h = createElanousShellRpcHandlers({
