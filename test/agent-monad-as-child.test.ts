// H5 Phase 3 · monad-as-child adapter tests.
//
// Uses a synthetic spawn factory so tests never fork a real monad
...
import {
  createMonadAsChildAdapter,
  registerDefaultMonadAsChildAdapter,
  _resetMonadAsChildSessionSeqForTesting,
  MONAD_AGENT_DEPTH_ENV,
  MONAD_AGENT_DEPTH_CAP_ENV,
} from '../src/agent/adapters/monad-as-child.js';
...
afterEach(() => {
  _resetMonadAsChildSessionSeqForTesting();
  delete process.env[MONAD_AGENT_DEPTH_ENV];
  delete process.env[MONAD_AGENT_DEPTH_CAP_ENV];
});

describe('createMonadAsChildAdapter · supports()', () => {
  test('matches monad + monad-child brand · acp/hybrid/auto modes', () => {
    const a = createMonadAsChildAdapter();
    expect(a.id).toBe('monad-as-child');
    expect(a.supports({ brand: 'monad' })).toBe(true);
    expect(a.supports({ brand: 'monad-child' })).toBe(true);
    expect(a.supports({ brand: 'monad', mode: 'acp' })).toBe(true);
    expect(a.supports({ brand: 'monad', mode: 'hybrid' })).toBe(true);
    expect(a.supports({ brand: 'monad', mode: 'auto' })).toBe(true);
  });
  ...
  test('rejects codex/claude/gemini and pty-direct mode', () => {
    ...
    expect(a.supports({ brand: 'monad', mode: 'pty-direct' })).toBe(false);
    expect(a.supports({ brand: 'monad', mode: 'native-sdk' })).toBe(false);
  });
});

describe('createMonadAsChildAdapter · launch()', () => {
  test('spawns "monad --acp-server" with default binary', async () => {
    const { spawn, children } = makeFakeSpawn();
    const a = createMonadAsChildAdapter({ _spawnFactory: spawn });
    const session = await a.launch({ brand: 'monad' });
    expect(children).toHaveLength(1);
    expect(children[0]!.argv).toEqual(['monad', '--acp-server']);
    expect(session.id).toMatch(/^emb-monad-as-child-/);
    expect(session.transports[0]!.kind).toBe('acp');
    expect(session.transports[0]!.label).toBe('monad-as-child');
  ...
  test('custom binary + extraArgs', async () => {
    ...
    const a = createMonadAsChildAdapter({
      _spawnFactory: spawn,
      monadBinary: '/custom/monad',
      extraArgs: ['--verbose'],
    });
    await a.launch({ brand: 'monad', extraArgs: ['--debug'] });
    expect(children[0]!.argv).toEqual(['/custom/monad', '--acp-server', '--verbose', '--debug']);
  });

  test('cwd + env propagate', async () => {
    const { spawn, children } = makeFakeSpawn();
    const a = createMonadAsChildAdapter({ _spawnFactory: spawn });
    await a.launch({ brand: 'monad', cwd: '/tmp/workspace', env: { FOO: 'bar' } });
...
describe('recursion depth cap', () => {
  test('depth bumps in child env · default cap=3 · first spawn has depth=1', async () => {
    const { spawn, children } = makeFakeSpawn();
    const a = createMonadAsChildAdapter({ _spawnFactory: spawn });
    await a.launch({ brand: 'monad' });
    expect(children[0]!.env[MONAD_AGENT_DEPTH_ENV]).toBe('1');
    expect(children[0]!.env[MONAD_AGENT_DEPTH_CAP_ENV]).toBe('3');
  });
  ...
  test('refuses to spawn when cap reached', async () => {
    ...
    let err: unknown;
    try {
      await a.launch({ brand: 'monad', env: { [MONAD_AGENT_DEPTH_ENV]: '3', [MONAD_AGENT_DEPTH_CAP_ENV]: '3' } });
    } catch (e) { err = e; }
  ...
  test('opts.depthCap override respected', async () => {
    const { spawn, children } = makeFakeSpawn();
    const a = createMonadAsChildAdapter({ _spawnFactory: spawn, depthCap: 5 });
    await a.launch({ brand: 'monad' });
    expect(children[0]!.env[MONAD_AGENT_DEPTH_CAP_ENV]).toBe('5');
  });

  test('env-level cap beats opts.depthCap', async () => {
    const { spawn, children } = makeFakeSpawn();
    const a = createMonadAsChildAdapter({ _spawnFactory: spawn, depthCap: 10 });
    await a.launch({ brand: 'monad', env: { [MONAD_AGENT_DEPTH_CAP_ENV]: '2' } });
    expect(children[0]!.env[MONAD_AGENT_DEPTH_CAP_ENV]).toBe('2');
  });
  ...
  test('chained recursion · depth increments', async () => {
    ...
    await a.launch({
      brand: 'monad',
      env: { [MONAD_AGENT_DEPTH_ENV]: '1', [MONAD_AGENT_DEPTH_CAP_ENV]: '3' },
    });
    expect(children[0]!.env[MONAD_AGENT_DEPTH_ENV]).toBe('1');
    expect(children[1]!.env[MONAD_AGENT_DEPTH_ENV]).toBe('2');
  });
});
...
describe('session lifecycle', () => {
  ...
  test('snapshot returns structured one-liner (not a terminal buffer)', async () => {
    ...
    expect(snap).toContain('monad-as-child');
    expect(snap).toContain('pid=');
    expect(snap).toContain('status=');
  });
});

describe('registerDefaultMonadAsChildAdapter', () => {
  test('registers into a registry · returns disposer', () => {
    const r = new AdapterRegistry();
    const { spawn } = makeFakeSpawn();
    const off = registerDefaultMonadAsChildAdapter(r, { _spawnFactory: spawn });
    expect(r.list().map((x) => x.id)).toEqual(['monad-as-child']);
    off();
