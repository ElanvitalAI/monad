// M3 (2026-04-28) — codex-app-server `monad/ui/*` envelope parity.
//
// Verifies that the agent intercepts `agent_thought_chunk` updates whose
// text matches the monad-extensions envelope and forwards them to a
...
function makeHarness(opts: { monadUiHandler?: CodexMonadUiHandler } = {}): Harness {
  ...
  const agent = new CodexAppServerAgent({
    ...
    cwd: '/tmp',
    monadUiHandler: opts.monadUiHandler,
...
describe('CodexAppServerAgent · M3 · envelope interception', () => {
  test('agent_thought_chunk with showModal envelope → handler called, onUpdate suppressed', async () => {
    const calls: Array<Parameters<CodexMonadUiHandler>[0]> = [];
    const updates: unknown[] = [];
    const h = makeHarness({ monadUiHandler: (params) => calls.push(params) });
    ...
    const env = formatMonadUiEnvelope({ method: 'showModal', payload });
  ...
  test('all 4 envelope methods (showModal / showToast / updateStatusPill / usage) intercepted', async () => {
    const calls: Array<Parameters<CodexMonadUiHandler>[0]> = [];
    const h = makeHarness({ monadUiHandler: (p) => calls.push(p) });
    const { threadId } = await startTurn(h, { onUpdate: () => {} });

    const showModalPayload: MonadUiShowModalPayload = {
      id: 'sm-1', kind: 'warn', title: 't', actions: [],
    };
    const showToastPayload: MonadUiShowToastPayload = {
      id: 'st-1', tone: 'success', text: 'done',
    };
    const updateStatusPillPayload: MonadUiUpdateStatusPillPayload = {
      id: 'pill-x', text: 'idle',
    };
    const usagePayload: MonadUiUsagePayload = {
      id: 'turn:1', inputTokens: 100, outputTokens: 200,
    };

    h.notify('item/reasoning/textDelta', {
      threadId, delta: formatMonadUiEnvelope({ method: 'showModal', payload: showModalPayload }),
    });
    h.notify('item/reasoning/textDelta', {
      threadId, delta: formatMonadUiEnvelope({ method: 'showToast', payload: showToastPayload }),
    });
    h.notify('item/reasoning/textDelta', {
      threadId, delta: formatMonadUiEnvelope({ method: 'updateStatusPill', payload: updateStatusPillPayload }),
    });
    h.notify('item/reasoning/textDelta', {
      threadId, delta: formatMonadUiEnvelope({ method: 'usage', payload: usagePayload }),
    });
    await tick();
  ...
  test('handler-absent run · envelope text passes through verbatim', async () => {
    // No monadUiHandler set — envelope should NOT be intercepted, the
    ...
    const payload: MonadUiShowToastPayload = { id: 't-1', tone: 'info', text: 'hi' };
    const env = formatMonadUiEnvelope({ method: 'showToast', payload });
    h.notify('item/reasoning/textDelta', { threadId, delta: env });
    await tick();
  ...
  test('handler throws → error logged but downstream onUpdate still suppressed (envelope was valid)', async () => {
    ...
    const env = formatMonadUiEnvelope({
      method: 'showToast',
    ...
    expect(logged.some((m) => m.includes('monadUiHandler throw'))).toBe(true);

    await agent.stop();
    void promptP;
  });

  test('runtime swap via setMonadUiHandler', async () => {
    const calls1: Array<Parameters<CodexMonadUiHandler>[0]> = [];
    const calls2: Array<Parameters<CodexMonadUiHandler>[0]> = [];
    const h = makeHarness({ monadUiHandler: (p) => calls1.push(p) });
    const { threadId } = await startTurn(h, { onUpdate: () => {} });

    h.notify('item/reasoning/textDelta', {
      threadId,
      delta: formatMonadUiEnvelope({
    ...
    h.agent.setMonadUiHandler((p) => calls2.push(p));
    h.notify('item/reasoning/textDelta', {
      threadId,
      delta: formatMonadUiEnvelope({
    ...
    h.agent.setMonadUiHandler(undefined);
    const updates: unknown[] = [];
    h.agent.setMonadUiHandler();
