...
import { summarizeTuneGate } from '../scripts/se-monad-self-tune.js';
...
describe('SE benchmark and tune integrity-gate consumption', () => {
  it.each([
    ...
    ['se-monad-self-tune.ts', "const gate = await runIntegrityGate(plan.worktreePath, { steps: ['test'], testArgs: gateArgs });"],
  ...
  it.each(['se-backend-bench.ts', 'se-monad-self-tune.ts'])('%s keeps main private and retains its local CLI entry point', (file) => {
  ...
  it('monad tune reports distinct pass and failure results without throwing', () => {
  ...
  it('monad tune preserves the zero-change false-positive distinction', () => {
